...
describe('fetchAnnotationWorkspace', () => {
  ...
  it('throws on non-ok response', async () => {
    ...
    await expect(fetchAnnotationWorkspace('proj-1', 'v-1', 'asset-1')).rejects.toThrow('404');
  });
});
...
describe('getMetricTone', () => {
  describe('precision', () => {
    it('returns green for >= 0.80', () => {
      expect(getMetricTone(0.8, 'precision')).toBe('green');
    ...
    it('returns amber for 0.60-0.79', () => {
      ...
      expect(getMetricTone(0.6, 'precision')).toBe('amber');
    });
    it('returns red for < 0.60', () => {
      expect(getMetricTone(0.59, 'precision')).toBe('red');
      expect(getMetricTone(0.3, 'precision')).toBe('red');
    });
...
- packages/contracts/src/**tests**/ (existing test files to avoid duplication)
  </read_first>
