...
export function GeneratedDocumentActionPanel({ documentId }: Props) {
  ...
  async function handleRenderDocx() {
    try {
      ...
      await renderGeneratedDocumentDocx(documentId);
      await load();

      setSuccessMessage("Đã render DOCX mới.");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Render DOCX thất bại.");
    } finally {
  ...
  async function handleConvertPdf() {
    try {
      ...
      await convertGeneratedDocumentPdf(documentId);
      await load();

      setSuccessMessage("Đã xuất PDF mới.");
    } catch (err) {
  ...
  async function handleDeleteSelected() {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Xóa file đã chọn thất bại.",
      );
    } finally {
      setActionLoading(null);
    }
  ...
  async function handleCleanupOldFiles() {
    const confirmed = window.confirm(
      "Dọn file cũ sẽ giữ lại DOCX mới nhất và PDF mới nhất, xóa toàn bộ file còn lại. Tiếp tục?",
    );
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Dọn file cũ thất bại.");
    } finally {
      setActionLoading(null);
    }
  }
  ...
  return (
    <section className="rounded-xl border bg-white p-4 shadow-sm">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <button
          type="button"
          onClick={handleRenderDocx}
          disabled={actionLoading !== null}
          className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-60"
        >
          {actionLoading === "DOCX" ? "Đang render DOCX..." : "Render DOCX"}
        </button>

        <button
          type="button"
          onClick={handleConvertPdf}
          disabled={actionLoading !== null || !latestDocx}
        ...
        >
          {actionLoading === "PDF" ? "Đang xuất PDF..." : "Xuất PDF"}
        </button>
      ...
      <div className="mt-5">
        <div className="mb-3 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm font-semibold text-slate-800">File đã tạo</p>
            <p className="text-xs text-slate-500">
              Chọn file để xóa thủ công hoặc dọn nhanh file cũ.
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              {actionLoading === "DELETE_SELECTED"
                ? "Đang xóa..."
                : `Xóa file đã chọn (${selectedFileIds.size})`}
            </button>
            ...
            >
              {actionLoading === "CLEANUP" ? "Đang dọn..." : "Dọn file cũ"}
            </button>
          </div>
        </div>
        ...
        {files.length ? (
          ...
          <p className="rounded-lg border border-dashed p-4 text-sm text-slate-500">
            Chưa có file nào. Hãy render DOCX trước.
          </p>
        )}
      </div>
    </section>
