...
const REQUIRED_FIELDS: RequiredField[] = [
  ...
  { section: "helper", field: "accusedName", label: "Tên bị can chính" },
  ...
  { section: "caseDecision", field: "prosecutionDecisionLegalBasisLine", label: "Căn cứ quyết định khởi tố vụ án" },
  { section: "accusedDecision", field: "prosecutionDecisionLegalBasisLine", label: "Căn cứ quyết định khởi tố bị can" },
...
function buildCaseDecisionLine(form: Bm148FormInputs): string {
  ...
  return ensureSentenceEnd(
    `Căn cứ Quyết định khởi tố vụ án hình sự số ${decisionCode} ${decisionDate} của ${agencyName} về tội “${offenseName}” quy định tại ${legalClause} ${legalArticle} Bộ luật Hình sự`,
    ";",
  );
}
...
function buildAccusedDecisionLine(form: Bm148FormInputs): string {
  ...
  const accusedName = form.helper.accusedName || form.person.fullName || "bị can";
  const offenseName = form.helper.offenseName || "Đánh bạc";
  const legalClause = form.helper.legalClause || "khoản 1";
  const legalArticle = form.helper.legalArticle || "Điều 321";

  return ensureSentenceEnd(
    `Căn cứ Quyết định khởi tố bị can số ${decisionCode} ${decisionDate} đối với ${accusedName}, về tội “${offenseName}” quy định tại ${legalClause} ${legalArticle} Bộ luật Hình sự`,
    ";",
  );
}
...
function syncDerivedFields(form: Bm148FormInputs): Bm148FormInputs {
  const accusedName = form.helper.accusedName || form.person.fullName || "bị can";
  const caseTitle = form.helper.caseTitle || `Vụ án ${form.helper.offenseName || "hình sự"}`;
  ...
  return {
    ...
    accusedDecision: {
      ...
      prosecutionDecisionLegalBasisLine: buildAccusedDecisionLine({
        ...form,
        helper: {
          ...form.helper,
          accusedName,
...
export function Bm148FormInputsPanel({
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
              Form này cho phép nhập tên bị can, tội danh và ngày tháng một lần.
              Các dòng căn cứ, Điều 1, Điều 3 và nơi nhận sẽ tự đồng bộ trước khi lưu.
            </p>
          </div>
      ...
      <SectionCard
        title="1. Cơ quan / quyết định"
        description="Ngày quyết định dùng bộ chọn ngày. Khi lưu, backend sẽ render ra ngày tháng tiếng Việt."
      >
      ...
      <SectionCard
        title="2. Dữ liệu đồng bộ: bị can / tội danh / quyết định"
        description="Sửa tên bị can hoặc tên tội tại đây, các dòng căn cứ và nội dung sẽ tự cập nhật."
      >
        <Field required label="Tên bị can chính" value={form.helper.accusedName} onChange={(value) => updateField("helper", "accusedName", value)} />
        ...
        <Field type="date" label="Ngày QĐ khởi tố vụ án" value={form.helper.caseDecisionDate} onChange={(value) => updateField("helper", "caseDecisionDate", value)} />
        ...
        <Field type="date" label="Ngày QĐ khởi tố bị can" value={form.helper.accusedDecisionDate} onChange={(value) => updateField("helper", "accusedDecisionDate", value)} />
      </SectionCard>
