...
function syncDerivedFields(form: Bm148FormInputs): Bm148FormInputs {
  ...
  const offense = form.source.offenseName.trim() || "Đánh bạc";
  const clause = form.source.legalClause.trim() || "khoản 1";
  const article = form.source.legalArticle.trim() || "Điều 321";
  const currentCaseTitle = form.source.caseTitle.trim();
  const locationSuffix = currentCaseTitle.match(/\s+tại\s+.+$/iu)?.[0] ?? "";
  const caseTitle = `Vụ án ${offense}${locationSuffix}`;
  const agency =
    form.source.investigationAgencyName.trim() ||
    stripRecipientLine(form.recipients.line1) ||
    "Cơ quan Cảnh sát điều tra Công an Thành phố Hồ Chí Minh";
  ...
  return {
    ...
    suspension: {
      ...
      reasonLine: name
        ? ensureEnd(`Xét thấy cần tạm đình chỉ vụ án hình sự đối với bị can ${name}`, ",")
        : "",
      article1Line: name
        ? ensureEnd(`Tạm đình chỉ vụ án hình sự đối với bị can ${name} trong ${caseTitle}`, ".")
        : "",
      article2ActionLine:
