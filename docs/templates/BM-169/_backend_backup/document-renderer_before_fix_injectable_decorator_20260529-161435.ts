...
// BM-169_OPTIONAL_PARAGRAPH_CLEANUP_START
function bm169DecodeXmlText(value: string): string {
  return value
    .replace(/<[^>]+>/gu, "")
    .replace(/&amp;/gu, "&")
    .replace(/&lt;/gu, "<")
...
function cleanupBm169OptionalBlankParagraphs(
  renderedBuffer: Buffer,
  payload: Record<string, any>,
): Buffer {
  if (payload?.template?.templateCode !== "BM-169") {
    return renderedBuffer;
  }

  const evidenceHandling =
    payload?.evidenceHandling && typeof payload.evidenceHandling === "object"
      ? (payload.evidenceHandling as Record<string, any>)
      : {};

  const hasOptionalDisabled =
    evidenceHandling.includeAccusedDecisionLine === false ||
    evidenceHandling.includeCaseSuspensionDecisionLine === false ||
    evidenceHandling.includeAccusedSuspensionDecisionLine === false;

  if (!hasOptionalDisabled) {
    return renderedBuffer;
  }
  ...
  const paragraphs = documentXml.match(/<w:p[\s\S]*?<\/w:p>/gu);

  if (!paragraphs || paragraphs.length === 0) {
    return renderedBuffer;
  }

  const paragraphTexts = paragraphs.map((paragraphXml) =>
    bm169DecodeXmlText(paragraphXml),
  );

  const officialIndex = paragraphTexts.findIndex((text) =>
    text.includes("VIỆN TRƯỞNG VIỆN KIỂM SÁT"),
  );
  ...
  const decisionIntroIndex = paragraphTexts.findIndex(
    (text, index) =>
      index > officialIndex &&
      text.replace(/\s+/gu, "").includes("QUYẾTĐỊNH:"),
  );

  if (decisionIntroIndex < 0 || decisionIntroIndex <= officialIndex) {
    return renderedBuffer;
  }

  const cleanedParagraphs = paragraphs.map((paragraphXml, index) => {
    const text = paragraphTexts[index];

    const isInsideLegalBasisBlock =
      index > officialIndex && index < decisionIntroIndex;

    if (isInsideLegalBasisBlock && !text) {
      return "";
    }
  ...
  const cleanedXml = documentXml.replace(/<w:p[\s\S]*?<\/w:p>/gu, () => {
    const replacement = cleanedParagraphs[paragraphIndex] ?? "";
    paragraphIndex += 1;
    return replacement;
  });
...
// BM-169_OPTIONAL_PARAGRAPH_CLEANUP_END
