...
const casePeople = await this.prisma.case_people.findMany({
  ...
  private sha256(filePath: string): string {
    ...
    if (templateCode === "BM-001") {
      ...
      const personFullName = "Đoàn Văn Dũng";
      ...
      target["accused"] = {
        ...accused,
        fullName: accused["fullName"] || personFullName,
      };

      const suspect = (target["suspect"] ?? {}) as Record<string, any>;
      target["suspect"] = {
        ...suspect,
        fullName: suspect["fullName"] || personFullName,
      };

      const defendant = (target["defendant"] ?? {}) as Record<string, any>;
      target["defendant"] = {
        ...defendant,
        fullName: defendant["fullName"] || personFullName,
      };
