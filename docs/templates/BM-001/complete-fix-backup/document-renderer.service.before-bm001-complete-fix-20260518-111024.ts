...
const casePeople = await this.prisma.case_people.findMany({
  ...
  private applyTemplateSpecificPayloadOverrides<TPayload extends Record<string, any>>(payload: TPayload): TPayload {
    ...
    if (templateCode === 'BM-001') {
      const localAgencyName = 'Viện kiểm sát nhân dân khu vực 7';
      const parentAgencyName = 'Viện kiểm sát nhân dân Thành phố Hồ Chí Minh';

      target['agency'] = {
        ...((target['agency'] ?? {}) as Record<string, any>),
        parentName: parentAgencyName,
        name: localAgencyName,
      };

      target['reception'] = {
        ...((target['reception'] ?? {}) as Record<string, any>),
        locationName: localAgencyName,
      };
      ...
      target['receiver'] = {
        ...
        departmentName: localAgencyName,
        signerName: 'Trần Thanh Nam',
      };

      target['informant'] = {
        ...((target['informant'] ?? {}) as Record<string, any>),
        fullName: 'Nguyễn Thị Hồng Hạnh',
  ...
  private scrubForbiddenTemplateDefaults(value: unknown, templateCode: string): unknown {
    const sanitizeText = (input: string): string => {
      let nextValue = input;

      // Không cho dữ liệu mẫu tên Huy lọt vào bất kỳ payload/output nào.
      nextValue = nextValue.replace(/\bHuy\b/gu, 'Nguyễn Thị Hồng Hạnh');

      // Chuẩn hóa chữ ký mặc định cũ nếu còn sót.
      nextValue = nextValue.replace(/Nguyễn\s+Thanh\s+Nam/gu, 'Trần Thanh Nam');

      if (templateCode === 'BM-001') {
        // Match cả khoảng trắng thường, tab, xuống dòng, NBSP trong text DOCX/JSON.
        nextValue = nextValue
          .replace(
    ...
    if (Array.isArray(value)) {
      for (let index = 0; index < value.length; index += 1) {
        value[index] = this.scrubForbiddenTemplateDefaults(value[index], templateCode);
      }
    ...
    if (typeof value === 'object') {
      ...
      for (const key of Object.keys(record)) {
        record[key] = this.scrubForbiddenTemplateDefaults(record[key], templateCode);
      }
