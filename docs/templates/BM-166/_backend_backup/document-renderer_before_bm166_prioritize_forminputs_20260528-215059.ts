...
// BM-059_AGENCY_BODY_CASE_FIX_END
    ...
    if (templateCode === "BM-166") {
      ...
      const bm166Agency = (target["agency"] ?? {}) as Record<string, any>;
      const bm166Document = (target["document"] ?? {}) as Record<string, any>;
      const bm166Case = (target["case"] ?? {}) as Record<string, any>;
      const bm166Helper = (target["helper"] ?? {}) as Record<string, any>;
      const bm166Source = (target["source"] ?? {}) as Record<string, any>;
      const bm166LegalBasis = (target["legalBasis"] ?? {}) as Record<string, any>;
      const bm166Recipients = (target["recipients"] ?? {}) as Record<string, any>;
      const bm166ReturnInvestigation =
        (target["returnInvestigation"] ?? {}) as Record<string, any>;
      ...
      const normalizedCaseTitle = caseTitle.replace(/^Vụ án\s+/iu, "");
