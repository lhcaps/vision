...
type WitnessStatementForm = {
  procedureArticlesLine: string;
  witnessName: string;
  witnessDob: string;
  witnessAddress: string;
  witnessJob: string;
  investigationReasonLine: string;
};
...
type Bm134Form = {
  ...
  witnessStatement: WitnessStatementForm;
...
type Bm134FormInputsPanelProps = {
...
const EMPTY_FORM: Bm134Form = {
  ...
  witnessStatement: {
    procedureArticlesLine:
      "Căn cứ các điều 36, 233 của Bộ luật Tố tụng hình sự;",
    witnessName: "",
    witnessDob: "",
    witnessAddress: "",
    witnessJob: "",
    investigationReasonLine: "",
  },
  recipients: {
...
function buildInvestigationReasonLine(form: Bm134Form): string {
  const data = form.witnessStatement;

  if (!data.investigationReasonLine.trim()) {
    return `Căn cứ kết quả điều tra vụ án và yêu cầu của Viện kiểm sát.`;
  }

  return data.investigationReasonLine.trim();
}
...
function normalizeFormInputs(payload: RenderPayload | null): Bm134Form {
  ...
  return {
    ...
    witnessStatement: {
      procedureArticlesLine:
        nested(payload, "witnessStatement.procedureArticlesLine") ||
        EMPTY_FORM.witnessStatement.procedureArticlesLine,

      witnessName:
        nested(payload, "witnessStatement.witnessName") ||
        nested(payload, "person.fullName") ||
        EMPTY_FORM.witnessStatement.witnessName,

      witnessDob:
        nested(payload, "witnessStatement.witnessDob") ||
        nested(payload, "person.dateOfBirth") ||
        EMPTY_FORM.witnessStatement.witnessDob,

      witnessAddress:
        nested(payload, "witnessStatement.witnessAddress") ||
        nested(payload, "person.address") ||
        EMPTY_FORM.witnessStatement.witnessAddress,

      witnessJob:
        nested(payload, "witnessStatement.witnessJob") ||
        EMPTY_FORM.witnessStatement.witnessJob,

      investigationReasonLine:
        nested(payload, "witnessStatement.investigationReasonLine") ||
        EMPTY_FORM.witnessStatement.investigationReasonLine,
    },
    recipients: {
      archiveLine:
...
function validateForm(form: Bm134Form): string[] {
  const required = [
    ...
    ["Căn cứ tố tụng", form.witnessStatement.procedureArticlesLine],
    ["Họ tên người làm chứng", form.witnessStatement.witnessName],
    ["Địa chỉ người làm chứng", form.witnessStatement.witnessAddress],
    ["Lý do điều tra", form.witnessStatement.investigationReasonLine],
...
function buildSaveBody(form: Bm134Form) {
  ...
  const witnessStatement = {
    procedureArticlesLine:
      form.witnessStatement.procedureArticlesLine,
    witnessName: form.witnessStatement.witnessName,
    witnessDob: form.witnessStatement.witnessDob,
    witnessAddress: form.witnessStatement.witnessAddress,
    witnessJob: form.witnessStatement.witnessJob,
    investigationReasonLine: buildInvestigationReasonLine(form),
  };
...
export function Bm134FormInputsPanel({
  ...
  const patch = <T extends keyof Bm134Form>(
    section: T,
    key: keyof Bm134Form[T],
    value: string,
  ) => {
    setForm((current) => ({
      ...current,
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải lại dữ liệu BM-134 từ backend.");
    } catch (err) {
  ...
  const handleFillSample = () => {
    setForm({
      ...
      witnessStatement: {
        ...EMPTY_FORM.witnessStatement,
      },
      recipients: { ...EMPTY_FORM.recipients },
      signature: { ...EMPTY_FORM.signature },
    });
    setError(null);
    setMessage("Đã điền dữ liệu mẫu BM-134.");
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu dữ liệu BM-134. Các dòng tự sinh đã đồng bộ.");
      await onSaved?.();
    } catch (err) {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      ...
      <SectionCard
        title="2. Thông tin người làm chứng"
      ...
      >
        <Field
          ...
          required
          multiline
          value={form.witnessStatement.procedureArticlesLine}
          onChange={(value) =>
            patch("witnessStatement", "procedureArticlesLine", value)
          }
        />
        ...
        <Field
          ...
          required
          value={form.witnessStatement.witnessName}
          onChange={(value) =>
            patch("witnessStatement", "witnessName", value)
          }
        />
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Field
            label="Ngày sinh"
            value={form.witnessStatement.witnessDob}
            onChange={(value) =>
              patch("witnessStatement", "witnessDob", value)
            }
          />

          <Field
            label="Nghề nghiệp"
            value={form.witnessStatement.witnessJob}
            onChange={(value) =>
              patch("witnessStatement", "witnessJob", value)
            }
          />
        </div>
        ...
        <Field
          label="Địa chỉ"
          required
          value={form.witnessStatement.witnessAddress}
          onChange={(value) =>
            patch("witnessStatement", "witnessAddress", value)
          }
        />

        <Field
          label="Lý do điều tra"
          required
          multiline
          value={form.witnessStatement.investigationReasonLine}
          onChange={(value) =>
            patch("witnessStatement", "investigationReasonLine", value)
          }
        />
      </SectionCard>
      ...
      >
        <Field
          label="Dòng căn cứ điều tra tự sinh"
          multiline
          readOnly
          value={buildInvestigationReasonLine(form)}
        />
      </SectionCard>
