...
type Bm092FormInputsPanelProps = {
...
const EMPTY_FORM: Bm092Form = {
  ...
  decisionSupplementApproval: {
    procedureArticlesLine:
      "Căn cứ các điều 36, 176, 177 của Bộ luật Tố tụng hình sự;",
...
function buildReasonLine(form: Bm092Form): string {
  ...
  if (!data.reasonLine.trim()) {
    return `Xét thấy việc bổ sung quyết định khởi tố bị can là cần thiết.`;
  }
...
function buildNewDecisionLine(form: Bm092Form): string {
  ...
  if (!data.newDecisionLine.trim()) {
    return `Bổ sung Quyết định khởi tố bị can đối với ${data.personName.trim()} về tội "${data.offenseName.trim()}".`;
  }
...
function validateForm(form: Bm092Form): string[] {
  const required = [
    ...
    ["Căn cứ tố tụng", form.decisionSupplementApproval.procedureArticlesLine],
    ["Quyết định cũ", form.decisionSupplementApproval.oldDecisionInfoLine],
    ["Lý do bổ sung", form.decisionSupplementApproval.reasonLine],
    ["Tên bị can", form.decisionSupplementApproval.personName],
    ["Tội danh", form.decisionSupplementApproval.offenseName],
...
function buildSaveBody(form: Bm092Form) {
  ...
  const decisionSupplementApproval = {
    procedureArticlesLine:
      form.decisionSupplementApproval.procedureArticlesLine,
...
export function Bm092FormInputsPanel({
  ...
  const patch = <T extends keyof Bm092Form>(
    section: T,
    key: keyof Bm092Form[T],
    value: string,
  ) => {
    setForm((current) => ({
      ...current,
  ...
  const handleFillSample = () => {
    setForm({
      ...
      decisionSupplementApproval: {
        ...EMPTY_FORM.decisionSupplementApproval,
      },
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <h2 className="mt-1 text-xl font-bold text-slate-950">
            QĐ phê chuẩn QĐ bổ sung QĐ khởi tố bị can
          </h2>
      ...
      <SectionCard
        title="2. Nội dung phê chuẩn bổ sung"
      ...
      >
        <Field
          ...
          onChange={(value) =>
            patch("decisionSupplementApproval", "procedureArticlesLine", value)
          }
        />

        <Field
          label="Quyết định khởi tố bị can cần bổ sung"
          required
          value={form.decisionSupplementApproval.oldDecisionInfoLine}
          onChange={(value) =>
            patch("decisionSupplementApproval", "oldDecisionInfoLine", value)
          }
        />
        ...
        <Field
          ...
          onChange={(value) =>
            patch("decisionSupplementApproval", "reasonLine", value)
          }
        />
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Field
            ...
            onChange={(value) =>
              patch("decisionSupplementApproval", "personName", value)
            }
          />
          ...
          <Field
            ...
            onChange={(value) =>
              patch("decisionSupplementApproval", "offenseName", value)
            }
          />
        </div>

        <Field
          label="Nội dung quyết định bổ sung (tự sinh nếu trống)"
          multiline
          value={form.decisionSupplementApproval.newDecisionLine}
          onChange={(value) =>
            patch("decisionSupplementApproval", "newDecisionLine", value)
          }
        />
      </SectionCard>
      ...
      >
        ...
        <Field
          ...
          multiline
          readOnly
          value={buildNewDecisionLine(form)}
        />
      </SectionCard>
