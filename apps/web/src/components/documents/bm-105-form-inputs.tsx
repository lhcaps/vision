...
const EMPTY_FORM: Bm105Form = {
  ...
  decision: { procedureArticlesLine: "Căn cứ các điều 36, 37 và 172 của Bộ luật Tố tụng hình sự;", caseTitle: "", offenseName: "", investigationDeadline: "", reasonLine: "" },
...
function buildSaveBody(form: Bm105Form) {
  ...
  return {
    ...
    decision: { procedureArticlesLine: form.decision.procedureArticlesLine, caseTitle: form.decision.caseTitle, offenseName: form.decision.offenseName, investigationDeadline: form.decision.investigationDeadline, reasonLine: buildReasonLine(form) },
...
export function Bm105FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = <T extends keyof Bm105Form>(section: T, key: keyof Bm105Form[T], value: string) => {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-105</p>
          <h2 className="mt-1 text-xl font-bold text-slate-950">QĐ không gia hạn thời hạn điều tra VAHS</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-105.</p>
        </div>
      ...
      <SectionCard title="2. Nội dung quyết định" description="Nhập thông tin vụ án và thời hạn điều tra.">
        ...
        <Field label="Thời hạn điều tra" value={form.decision.investigationDeadline} onChange={v => patch("decision", "investigationDeadline", v)} />
