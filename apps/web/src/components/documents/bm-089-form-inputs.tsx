...
const EMPTY_FORM: Bm089Form = {
  ...
  document: { documentCode: "89/QĐ-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
  official: { issuerTitle: "VIỆN TRƯỞNG VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7" },
  cancelledDecision: {
    cancelledDocumentCode: "", cancelledDocumentDate: "", cancelledDocumentAgency: "",
    caseTitle: "", offenseName: "", reasonLine: "",
  },
...
function buildIssuePlaceAndDateLine(form: Bm089Form): string {
...
function buildReasonLine(form: Bm089Form): string {
  ...
  return `Xét thấy QĐ số ${d.cancelledDocumentCode.trim()}${dateText ? ` ${dateText}` : ""} của ${d.cancelledDocumentAgency.trim()} về việc tách vụ án hình sự ${d.caseTitle.trim()} về tội "${d.offenseName.trim()}" không có căn cứ và trái pháp luật;`;
}
...
function normalizeFormInputs(payload: RenderPayload | null): Bm089Form {
  ...
  return {
    ...
    cancelledDecision: {
      cancelledDocumentCode: nested(payload, "cancelledDecision.cancelledDocumentCode") || nested(payload, "cancelledDocument.documentCode") || "",
      cancelledDocumentDate: nested(payload, "cancelledDecision.cancelledDocumentDate") || nested(payload, "cancelledDocument.issueDate") || "",
      cancelledDocumentAgency: nested(payload, "cancelledDecision.cancelledDocumentAgency") || nested(payload, "cancelledDocument.agency") || "",
...
function validateForm(form: Bm089Form): string[] {
  const reqs: [string, string][] = [
    ...
    ["Số QĐ bị huỷ", form.cancelledDecision.cancelledDocumentCode], ["Tên vụ án", form.cancelledDecision.caseTitle],
...
function buildSaveBody(form: Bm089Form) {
  ...
  return {
    ...
    cancelledDecision: { cancelledDocumentCode: form.cancelledDecision.cancelledDocumentCode, cancelledDocumentDate: toSlashDateText(cancelledDateIso), cancelledDocumentAgency: form.cancelledDecision.cancelledDocumentAgency, caseTitle: form.cancelledDecision.caseTitle, offenseName: form.cancelledDecision.offenseName, reasonLine: buildReasonLine(form) },
...
function SectionCard({ title, description, children }: { title: string; description?: string; children: ReactNode }) {
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="mb-4"><h3 className="text-sm font-bold uppercase tracking-[0.12em] text-slate-800">{title}</h3>
        {description ? <p className="mt-1 text-sm leading-6 text-slate-500">{description}</p> : null}</div>
      <div className="grid gap-4">{children}</div>
    </section>
  );
}

function Field({ label, value, onChange, required, multiline, type = "text", readOnly }: {
  label: string; value: string; onChange?: (v: string) => void; required?: boolean;
  multiline?: boolean; type?: "text" | "date"; readOnly?: boolean;
}) {
  ...
  return (
    <label className="grid gap-1.5">
      ...
      {multiline ? (<textarea className={`${cls} min-h-[88px] ${readOnly ? "bg-slate-100 text-slate-700" : "bg-white"}`} value={value} readOnly={readOnly} onChange={e => onChange?.(e.target.value)} />)
        : (<input className={`${cls} ${readOnly ? "bg-slate-100 text-slate-700" : "bg-white"}`} value={value} type={type} readOnly={readOnly} onChange={e => onChange?.(e.target.value)} />)}
    </label>
  );
}

export function Bm089FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = <T extends keyof Bm089Form>(section: T, key: keyof Bm089Form[T], value: string) => {
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải dữ liệu BM-089 từ backend.");
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu BM-089 thành công.");
      await onSaved?.();
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-089</p>
          <h2 className="mt-1 text-xl font-bold text-slate-950">QĐ huỷ bỏ QĐ tách vụ án hình sự</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-089.</p>
        </div>
      ...
      <SectionCard title="2. Quyết định bị huỷ bỏ" description="Nhập thông tin QĐ tách vụ án hình sự cần huỷ bỏ.">
