...
type ExpertForensicForm = {
  procedureArticlesLine: string;
  personName: string;
  evidenceDescription: string;
  expertRequirementLine: string;
};
...
type Bm126Form = {
  ...
  expertForensic: ExpertForensicForm;
...
type Bm126FormInputsPanelProps = {
...
const EMPTY_FORM: Bm126Form = {
  ...
  expertForensic: {
    procedureArticlesLine:
      "Căn cứ các điều 36, 206, 207 của Bộ luật Tố tụng hình sự;",
    personName: "",
    evidenceDescription: "",
    expertRequirementLine: "",
  },
  recipients: {
...
function buildExpertRequirementLine(form: Bm126Form): string {
  const data = form.expertForensic;

  if (!data.expertRequirementLine.trim()) {
    return `Yêu cầu giám định viên tiến hành giám định ${data.evidenceDescription.trim()} theo quy định pháp luật.`;
  }

  return data.expertRequirementLine.trim();
}
...
function normalizeFormInputs(payload: RenderPayload | null): Bm126Form {
  ...
  return {
    ...
    expertForensic: {
      procedureArticlesLine:
        nested(payload, "expertForensic.procedureArticlesLine") ||
        EMPTY_FORM.expertForensic.procedureArticlesLine,

      personName:
        nested(payload, "expertForensic.personName") ||
        nested(payload, "person.fullName") ||
        EMPTY_FORM.expertForensic.personName,

      evidenceDescription:
        nested(payload, "expertForensic.evidenceDescription") ||
        EMPTY_FORM.expertForensic.evidenceDescription,

      expertRequirementLine:
        nested(payload, "expertForensic.expertRequirementLine") ||
        EMPTY_FORM.expertForensic.expertRequirementLine,
    },
    recipients: {
      archiveLine:
...
function validateForm(form: Bm126Form): string[] {
  const required = [
    ...
    ["Căn cứ tố tụng", form.expertForensic.procedureArticlesLine],
    ["Tên người/đối tượng giám định", form.expertForensic.personName],
    ["Mô tả tang vật cần giám định", form.expertForensic.evidenceDescription],
...
function buildSaveBody(form: Bm126Form) {
  ...
  const expertForensic = {
    procedureArticlesLine:
      form.expertForensic.procedureArticlesLine,
    personName: form.expertForensic.personName,
    evidenceDescription: form.expertForensic.evidenceDescription,
    expertRequirementLine: buildExpertRequirementLine(form),
  };
...
export function Bm126FormInputsPanel({
  ...
  const patch = <T extends keyof Bm126Form>(
    section: T,
    key: keyof Bm126Form[T],
    value: string,
  ) => {
    setForm((current) => ({
      ...current,
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải lại dữ liệu BM-126 từ backend.");
    } catch (err) {
  ...
  const handleFillSample = () => {
    setForm({
      ...
      expertForensic: {
        ...EMPTY_FORM.expertForensic,
      },
      recipients: { ...EMPTY_FORM.recipients },
      signature: { ...EMPTY_FORM.signature },
    });
    setError(null);
    setMessage("Đã điền dữ liệu mẫu BM-126.");
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu dữ liệu BM-126. Các dòng tự sinh đã đồng bộ.");
      await onSaved?.();
    } catch (err) {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      ...
      <SectionCard
        title="2. Nội dung trưng cầu giám định"
      ...
      >
        <Field
          ...
          required
          multiline
          value={form.expertForensic.procedureArticlesLine}
          onChange={(value) =>
            patch("expertForensic", "procedureArticlesLine", value)
          }
        />

        <Field
          label="Tên người/đối tượng giám định"
          required
          value={form.expertForensic.personName}
          onChange={(value) =>
            patch("expertForensic", "personName", value)
          }
        />

        <Field
          label="Mô tả tang vật/hiện vật cần giám định"
          required
          multiline
          value={form.expertForensic.evidenceDescription}
          onChange={(value) =>
            patch("expertForensic", "evidenceDescription", value)
          }
        />

        <Field
          label="Yêu cầu giám định (tự sinh nếu trống)"
          multiline
          value={form.expertForensic.expertRequirementLine}
          onChange={(value) =>
            patch("expertForensic", "expertRequirementLine", value)
          }
        />
      </SectionCard>
      ...
      >
        <Field
          label="Dòng yêu cầu giám định tự sinh"
          multiline
          readOnly
          value={buildExpertRequirementLine(form)}
        />
      </SectionCard>
