...
export function Bm076FormInputsPanel({ documentId, onSaved }: Bm076FormInputsPanelProps) {
  ...
  return (
    <div className="space-y-5">
      ...
      <SectionCard title="4. Thông tin thay đổi người phiên dịch">
        <Field required label="Người phiên dịch cũ" value={form.changeInfo.oldPersonName} onChange={(v) => updateField("changeInfo", "oldPersonName", v)} />
        <Field label="Ngôn ngữ người cũ" value={form.changeInfo.oldPersonLanguage} onChange={(v) => updateField("changeInfo", "oldPersonLanguage", v)} />
        <Field required label="Người phiên dịch mới" value={form.changeInfo.newPersonName} onChange={(v) => updateField("changeInfo", "newPersonName", v)} />
        <Field label="Ngôn ngữ người mới" value={form.changeInfo.newPersonLanguage} onChange={(v) => updateField("changeInfo", "newPersonLanguage", v)} />
