...
type RequestForm = {
  ...
  arrestWarrantCode: string;
  arrestWarrantDate: string;
  reasonLine: string;
};
...
const EMPTY_FORM: Bm106Form = {
  ...
  document: { documentCode: "106/YCT-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
  official: { issuerTitle: "VIỆN TRƯỞNG VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7" },
  request: { procedureArticlesLine: "Căn cứ các điều 36, 232 và 233 của Bộ luật Tố tụng hình sự;", caseTitle: "", offenseName: "", accusedFullName: "", accusedDateOfBirth: "", accusedAddress: "", arrestWarrantCode: "", arrestWarrantDate: "", reasonLine: "" },
...
function buildReasonLine(form: Bm106Form): string {
  ...
  return `Qua xét nội dung vụ án ${r.caseTitle.trim()}, bị can ${r.accusedFullName.trim()}${r.accusedDateOfBirth ? `, sinh năm ${cleanText(r.accusedDateOfBirth)}` : ""}${r.accusedAddress ? `, địa chỉ: ${r.accusedAddress.trim()}` : ""} bị khởi tố về tội "${r.offenseName.trim()}" theo QĐ số ${r.arrestWarrantCode.trim()}${r.arrestWarrantDate ? ` ngày ${cleanText(r.arrestWarrantDate)}` : ""}. Hiện bị can đang bỏ trốn, để phục vụ điều tra cần yêu cầu truy nã bị can.`;
}
...
function normalizeFormInputs(payload: RenderPayload | null): Bm106Form {
  ...
  return {
    ...
    request: {
      ...
      accusedDateOfBirth: nested(payload, "request.accusedDateOfBirth") || nested(payload, "accused.dateOfBirth") || "",
      accusedAddress: nested(payload, "request.accusedAddress") || nested(payload, "accused.address") || "",
      arrestWarrantCode: nested(payload, "request.arrestWarrantCode") || "",
      arrestWarrantDate: nested(payload, "request.arrestWarrantDate") || "",
...
function validateForm(form: Bm106Form): string[] {
  const reqs: [string, string][] = [
    ...
    ["Số QĐ khởi tố", form.request.arrestWarrantCode],
...
function buildSaveBody(form: Bm106Form) {
  ...
  return {
    ...
    request: { procedureArticlesLine: form.request.procedureArticlesLine, caseTitle: form.request.caseTitle, offenseName: form.request.offenseName, accusedFullName: form.request.accusedFullName, accusedDateOfBirth: form.request.accusedDateOfBirth, accusedAddress: form.request.accusedAddress, arrestWarrantCode: form.request.arrestWarrantCode, arrestWarrantDate: form.request.arrestWarrantDate, reasonLine: buildReasonLine(form) },
...
export function Bm106FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = <T extends keyof Bm106Form>(section: T, key: keyof Bm106Form[T], value: string) => {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-106</p>
          <h2 className="mt-1 text-xl font-bold text-slate-950">Yêu cầu truy nã bị can</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-106.</p>
        </div>
      ...
      <SectionCard title="2. Thông tin truy nã" description="Nhập thông tin bị can cần truy nã.">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Field label="Năm sinh" value={form.request.accusedDateOfBirth} onChange={v => patch("request", "accusedDateOfBirth", v)} />
          <Field label="Địa chỉ" value={form.request.accusedAddress} onChange={v => patch("request", "accusedAddress", v)} />
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Số QĐ khởi tố" required value={form.request.arrestWarrantCode} onChange={v => patch("request", "arrestWarrantCode", v)} />
          <Field label="Ngày QĐ khởi tố" value={form.request.arrestWarrantDate} onChange={v => patch("request", "arrestWarrantDate", v)} />
        </div>
        <Field label="Lý do truy nã tự sinh" multiline readOnly value={buildReasonLine(form)} />
      </SectionCard>
