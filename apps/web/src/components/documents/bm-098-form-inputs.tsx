...
const EMPTY_FORM: Bm098Form = {
  ...
  request: { procedureArticlesLine: "Căn cứ các điều 36, 37 và 167 của Bộ luật Tố tụng hình sự;", caseTitle: "", offenseName: "", accusedFullName: "", originalDecisionCode: "", originalDecisionDate: "", changeReason: "", reasonLine: "" },
...
function validateForm(form: Bm098Form): string[] {
  const reqs: [string, string][] = [
    ...
    ["Số QĐ gốc", form.request.originalDecisionCode], ["Lý do thay đổi", form.request.changeReason],
...
function buildSaveBody(form: Bm098Form) {
  ...
  return {
    ...
    request: { procedureArticlesLine: form.request.procedureArticlesLine, caseTitle: form.request.caseTitle, offenseName: form.request.offenseName, accusedFullName: form.request.accusedFullName, originalDecisionCode: form.request.originalDecisionCode, originalDecisionDate: form.request.originalDecisionDate, changeReason: form.request.changeReason, reasonLine: buildReasonLine(form) },
...
export function Bm098FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = <T extends keyof Bm098Form>(section: T, key: keyof Bm098Form[T], value: string) => {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-098</p>
          <h2 className="mt-1 text-xl font-bold text-slate-950">Yêu cầu ra QĐ thay đổi quyết định khởi tố bị can</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-098.</p>
        </div>
      ...
      <SectionCard title="2. Nội dung yêu cầu" description="Nhập thông tin QĐ khởi tố bị can cần thay đổi.">
        ...
        <Field label="Lý do thay đổi" required value={form.request.changeReason} onChange={v => patch("request", "changeReason", v)} />
