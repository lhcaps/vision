...
const EMPTY_FORM: Bm122Form = {
  ...
  decision: { procedureArticlesLine: "Căn cứ các điều 36, 37 và 116 của Bộ luật Tố tụng hình sự;", caseTitle: "", orderCode: "", orderDate: "", orderAgency: "", propertyDescription: "", refusalReason: "", reasonLine: "" },
...
function validateForm(form: Bm122Form): string[] {
  const reqs: [string, string][] = [
    ...
    ["Lý do không phê chuẩn", form.decision.refusalReason],
...
function buildSaveBody(form: Bm122Form) {
  ...
  return {
    ...
    decision: { procedureArticlesLine: form.decision.procedureArticlesLine, caseTitle: form.decision.caseTitle, orderCode: form.decision.orderCode, orderDate: form.decision.orderDate, orderAgency: form.decision.orderAgency, propertyDescription: form.decision.propertyDescription, refusalReason: form.decision.refusalReason, reasonLine: buildReasonLine(form) },
...
export function Bm122FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = <T extends keyof Bm122Form>(section: T, key: keyof Bm122Form[T], value: string) => {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-122</p>
          <h2 className="mt-1 text-xl font-bold text-slate-950">QĐ không phê chuẩn Lệnh thu giữ thư tín, điện tín, bưu kiện, bưu phẩm</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-122.</p>
        </div>
      ...
      <SectionCard title="2. Thông tin không phê chuẩn Lệnh" description="Nhập thông tin Lệnh không được phê chuẩn.">
        ...
        <Field label="Lý do không phê chuẩn" required multiline value={form.decision.refusalReason} onChange={v => patch("decision", "refusalReason", v)} />
