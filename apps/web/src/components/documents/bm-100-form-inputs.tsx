...
const EMPTY_FORM: Bm100Form = {
  ...
  request: { procedureArticlesLine: "Căn cứ các điều 36, 37 và 167 của Bộ luật Tố tụng hình sự;", caseTitle: "", offenseName: "", accusedFullName: "", originalDecisionCode: "", originalDecisionDate: "", supplementReason: "", reasonLine: "" },
...
function validateForm(form: Bm100Form): string[] {
  const reqs: [string, string][] = [
    ...
    ["Số QĐ gốc", form.request.originalDecisionCode], ["Lý do bổ sung", form.request.supplementReason],
...
function buildSaveBody(form: Bm100Form) {
  ...
  return {
    ...
    request: { procedureArticlesLine: form.request.procedureArticlesLine, caseTitle: form.request.caseTitle, offenseName: form.request.offenseName, accusedFullName: form.request.accusedFullName, originalDecisionCode: form.request.originalDecisionCode, originalDecisionDate: form.request.originalDecisionDate, supplementReason: form.request.supplementReason, reasonLine: buildReasonLine(form) },
...
export function Bm100FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = <T extends keyof Bm100Form>(section: T, key: keyof Bm100Form[T], value: string) => {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-100</p>
          <h2 className="mt-1 text-xl font-bold text-slate-950">Yêu cầu ra QĐ bổ sung QĐ khởi tố bị can</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-100.</p>
        </div>
      ...
      <SectionCard title="2. Nội dung yêu cầu" description="Nhập thông tin QĐ khởi tố bị can cần bổ sung.">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Số QĐ khởi tố gốc" required value={form.request.originalDecisionCode} onChange={v => patch("request", "originalDecisionCode", v)} />
          <Field label="Ngày QĐ khởi tố gốc" value={form.request.originalDecisionDate} onChange={v => patch("request", "originalDecisionDate", v)} />
        </div>
        <Field label="Lý do bổ sung" required value={form.request.supplementReason} onChange={v => patch("request", "supplementReason", v)} />
        <Field label="Lý do yêu cầu tự sinh" multiline readOnly value={buildReasonLine(form)} />
      </SectionCard>
