...
type InvestigationExtensionForm = {
  procedureArticlesLine: string;
  extensionCount: string;
  previousDecisionInfoLine: string;
  caseTitle: string;
  offenseName: string;
  currentStatusLine: string;
  extensionReasonLine: string;
  newDeadlineLine: string;
};
...
type Bm104Form = {
  ...
  investigationExtension: InvestigationExtensionForm;
...
type Bm104FormInputsPanelProps = {
...
const EMPTY_FORM: Bm104Form = {
  ...
  investigationExtension: {
    procedureArticlesLine: "Căn cứ Điều 187 của Bộ luật Tố tụng hình sự;",
    extensionCount: "lần thứ nhất",
    previousDecisionInfoLine: "Quyết định số 85/QĐ-VKSKV7 ngày 01/05/2026",
...
function buildCurrentStatusLine(form: Bm104Form): string {
  const data = form.investigationExtension;

  if (!data.currentStatusLine.trim()) {
    return `Cơ quan Cảnh sát điều tra đang tiến hành điều tra vụ án ${data.caseTitle.trim()} về tội ${data.offenseName.trim()}.`;
  }

  return data.currentStatusLine.trim();
}

function buildNewDeadlineLine(form: Bm104Form): string {
  const data = form.investigationExtension;

  if (!data.newDeadlineLine.trim()) {
    return `Gia hạn thời hạn điều tra đến hết ngày [số ngày] tháng [số tháng] năm [số năm].`;
  }

  return data.newDeadlineLine.trim();
}
  ...
  return {
    ...
    investigationExtension: {
      procedureArticlesLine:
        nested(payload, "investigationExtension.procedureArticlesLine") ||
        EMPTY_FORM.investigationExtension.procedureArticlesLine,

      extensionCount:
        nested(payload, "investigationExtension.extensionCount") ||
        EMPTY_FORM.investigationExtension.extensionCount,

      previousDecisionInfoLine:
        nested(payload, "investigationExtension.previousDecisionInfoLine") ||
        EMPTY_FORM.investigationExtension.previousDecisionInfoLine,

      caseTitle:
        nested(payload, "investigationExtension.caseTitle") ||
        nested(payload, "case.caseTitle") ||
        EMPTY_FORM.investigationExtension.caseTitle,

      offenseName:
        nested(payload, "investigationExtension.offenseName") ||
        nested(payload, "offense.offenseName") ||
        EMPTY_FORM.investigationExtension.offenseName,

      currentStatusLine:
        nested(payload, "investigationExtension.currentStatusLine") ||
        EMPTY_FORM.investigationExtension.currentStatusLine,

      extensionReasonLine:
        nested(payload, "investigationExtension.extensionReasonLine") ||
        EMPTY_FORM.investigationExtension.extensionReasonLine,

      newDeadlineLine:
        nested(payload, "investigationExtension.newDeadlineLine") ||
        EMPTY_FORM.investigationExtension.newDeadlineLine,
    },
    recipients: {
      archiveLine:
...
function validateForm(form: Bm104Form): string[] {
  const required = [
    ...
    ["Căn cứ tố tụng", form.investigationExtension.procedureArticlesLine],
    ["Lần gia hạn", form.investigationExtension.extensionCount],
    ["Quyết định gia hạn trước", form.investigationExtension.previousDecisionInfoLine],
    ["Tên vụ án", form.investigationExtension.caseTitle],
    ["Tội danh", form.investigationExtension.offenseName],
    ["Lý do gia hạn", form.investigationExtension.extensionReasonLine],
...
function buildSaveBody(form: Bm104Form) {
  ...
  const investigationExtension = {
    procedureArticlesLine:
      form.investigationExtension.procedureArticlesLine,
    extensionCount: form.investigationExtension.extensionCount,
    previousDecisionInfoLine:
      form.investigationExtension.previousDecisionInfoLine,
    caseTitle: form.investigationExtension.caseTitle,
    offenseName: form.investigationExtension.offenseName,
    currentStatusLine: buildCurrentStatusLine(form),
    extensionReasonLine: form.investigationExtension.extensionReasonLine,
    newDeadlineLine: buildNewDeadlineLine(form),
  };
...
export function Bm104FormInputsPanel({
  ...
  const patch = <T extends keyof Bm104Form>(
    section: T,
    key: keyof Bm104Form[T],
    value: string,
  ) => {
    setForm((current) => ({
      ...current,
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải lại dữ liệu BM-104 từ backend.");
    } catch (err) {
  ...
  const handleFillSample = () => {
    setForm({
      ...
      investigationExtension: {
        ...EMPTY_FORM.investigationExtension,
      },
      recipients: { ...EMPTY_FORM.recipients },
      signature: { ...EMPTY_FORM.signature },
    });
    setError(null);
    setMessage("Đã điền dữ liệu mẫu BM-104.");
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu dữ liệu BM-104. Các dòng tự sinh đã đồng bộ.");
      await onSaved?.();
    } catch (err) {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <h2 className="mt-1 text-xl font-bold text-slate-950">
            QĐ gia hạn thời hạn điều tra vụ án hình sự
          </h2>
      ...
      <SectionCard
        title="2. Nội dung gia hạn điều tra"
      ...
      >
        <Field
          ...
          required
          multiline
          value={form.investigationExtension.procedureArticlesLine}
          onChange={(value) =>
            patch("investigationExtension", "procedureArticlesLine", value)
          }
        />
        ...
        <Field
          label="Lần gia hạn"
          required
          value={form.investigationExtension.extensionCount}
          onChange={(value) =>
            patch("investigationExtension", "extensionCount", value)
          }
        />

        <Field
          label="Quyết định gia hạn trước đó"
          required
          value={form.investigationExtension.previousDecisionInfoLine}
          onChange={(value) =>
            patch("investigationExtension", "previousDecisionInfoLine", value)
          }
        />
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Field
            label="Tên vụ án"
            required
            value={form.investigationExtension.caseTitle}
            onChange={(value) =>
              patch("investigationExtension", "caseTitle", value)
            }
          />
          ...
          <Field
            label="Tội danh"
            required
            value={form.investigationExtension.offenseName}
            onChange={(value) =>
              patch("investigationExtension", "offenseName", value)
            }
          />
        </div>
        ...
        <Field
          ...
          required
          multiline
          value={form.investigationExtension.extensionReasonLine}
          onChange={(value) =>
            patch("investigationExtension", "extensionReasonLine", value)
          }
        />
      </SectionCard>
      ...
      >
        <Field
          label="Dòng tình trạng điều tra tự sinh"
          multiline
          readOnly
          value={buildCurrentStatusLine(form)}
        />

        <Field
          label="Dòng thời hạn mới tự sinh"
          multiline
          readOnly
          value={buildNewDeadlineLine(form)}
        />
      </SectionCard>
