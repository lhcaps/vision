...
type Bm160Form = {
  ...
  content: { 
    postingLocation: string; postingDate: string; postingTime: string; postingContent: string;
    article1Line: string; article2Line: string;
  };
  recipients: { line1: string; line2: string; archiveLine: string };
  signature: { signMode: string; positionTitle: string; signerName: string };
};

type Bm160Props = { documentId: string | number; onSaved?: () => void };
...
const EMPTY_FORM: Bm160Form = {
  ...
  content: { postingLocation: "", postingDate: "", postingTime: "08:00", postingContent: "", article1Line: "", article2Line: "" },
...
function normalizeForm(payload: Record<string, unknown> | null): Bm160Form {
  ...
  return {
    ...
    content: { postingLocation: nested(payload, "content.postingLocation") || "", postingDate: nested(payload, "content.postingDate") || "", postingTime: nested(payload, "content.postingTime") || "08:00", postingContent: nested(payload, "content.postingContent") || "", article1Line: nested(payload, "content.article1Line") || "", article2Line: nested(payload, "content.article2Line") || "" },
...
export function Bm160FormInputsPanel({ documentId, onSaved }: Bm160Props) {
  const [form, setForm] = useState<Bm160Form>(EMPTY_FORM);
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const patch = (section: keyof Bm160Form, key: string, value: string) => setForm(f => ({ ...f, [section]: { ...f[section], [key]: value } }));
  ...
  return (<div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm"><div className="flex items-center justify-between border-b border-slate-200 pb-4"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-160</p><h2 className="mt-1 text-xl font-bold text-slate-950">Biên bản niêm yết công khai văn bản tố tụng</h2></div><div className="flex gap-2"><button className="rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50" onClick={() => void reload()}>Tải lại</button><button className="rounded-xl bg-blue-600 px-3.5 py-2.5 text-sm font-bold text-white hover:bg-blue-700" onClick={() => void handleSave()} disabled={saving}>{saving ? "Đang lưu..." : "Lưu dữ liệu"}</button></div></div>{msg ? <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-700">{msg}</div> : null}{error ? <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-700">{error}</div> : null}<SectionCard title="1. Header biểu mẫu"><Field label="Viện kiểm sát cấp trên" value={form.agency.parentName} onChange={v => patch("agency", "parentName", v)} /><Field label="Viện kiểm sát ban hành" value={form.agency.name} onChange={v => patch("agency", "name", v)} /><Field label="Số biên bản" value={form.document.documentCode} onChange={v => patch("document", "documentCode", v)} /><Field label="Địa danh" value={form.agency.issuePlace} onChange={v => patch("agency", "issuePlace", v)} /><Field label="Ngày lập" type="date" value={form.document.issueDate} onChange={v => patch("document", "issueDate", v)} /><Field label="Chủ thể ban hành" value={form.official.issuerTitle} onChange={v => patch("official", "issuerTitle", v)} className="md:col-span-2" /></SectionCard><SectionCard title="2. Thông tin niêm yết"><Field label="Địa điểm niêm yết" value={form.content.postingLocation} onChange={v => patch("content", "postingLocation", v)} className="md:col-span-2" /><Field label="Ngày niêm yết" type="date" value={form.content.postingDate} onChange={v => patch("content", "postingDate", v)} /><Field label="Giờ niêm yết" type="time" value={form.content.postingTime} onChange={v => patch("content", "postingTime", v)} /><Field label="Nội dung văn bản được niêm yết" multiline value={form.content.postingContent} onChange={v => patch("content", "postingContent", v)} className="md:col-span-2" /><Field label="Điều 1" multiline value={form.content.article1Line} onChange={v => patch("content", "article1Line", v)} className="md:col-span-2" /><Field label="Điều 2" multiline value={form.content.article2Line} onChange={v => patch("content", "article2Line", v)} className="md:col-span-2" /></SectionCard><SectionCard title="3. Nơi nhận"><Field label="Nơi nhận 1" value={form.recipients.line1} onChange={v => patch("recipients", "line1", v)} /><Field label="Nơi nhận 2" value={form.recipients.line2} onChange={v => patch("recipients", "line2", v)} /><Field label="Lưu hồ sơ" value={form.recipients.archiveLine} onChange={v => patch("recipients", "archiveLine", v)} className="md:col-span-2" /></SectionCard><SectionCard title="4. Chữ ký"><Field label="Chế độ ký" value={form.signature.signMode} onChange={v => patch("signature", "signMode", v)} /><Field label="Chức vụ ký" value={form.signature.positionTitle} onChange={v => patch("signature", "positionTitle", v)} /><Field label="Người ký" value={form.signature.signerName} onChange={v => patch("signature", "signerName", v)} /></SectionCard></div>);
}
