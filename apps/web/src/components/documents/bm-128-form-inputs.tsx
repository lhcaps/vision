...
const EMPTY_FORM: Bm128Form = {
  ...
  document: { documentCode: "128/TB-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
  official: { issuerTitle: "VIỆN TRƯỞNG VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7" },
  notification: { requestCode: "", requestDate: "", requestAgency: "", appraisalResult: "", propertyDescription: "", propertyValue: "", appraiserName: "", appraisalDate: "" },
...
function normalizeFormInputs(payload: RenderPayload | null): Bm128Form {
  ...
  return {
    ...
    notification: {
      ...
      requestAgency: nested(payload, "notification.requestAgency") || "",
      appraisalResult: nested(payload, "notification.appraisalResult") || "",
      propertyDescription: nested(payload, "notification.propertyDescription") || "",
      propertyValue: nested(payload, "notification.propertyValue") || "",
      appraiserName: nested(payload, "notification.appraiserName") || "",
      appraisalDate: nested(payload, "notification.appraisalDate") || "",
    },
...
function validateForm(form: Bm128Form): string[] {
  const reqs: [string, string][] = [
    ...
    ["Số thông báo", form.document.documentCode], ["Địa danh", form.document.issuePlace],
    ["Ngày ban hành", form.document.issueDateIso], ["Chủ thể ban hành", form.official.issuerTitle],
    ["Số yêu cầu", form.notification.requestCode], ["Cơ quan thực hiện", form.notification.requestAgency],
    ["Mô tả tài sản", form.notification.propertyDescription], ["Kết quả giám định/định giá", form.notification.appraisalResult],
...
function buildSaveBody(form: Bm128Form) {
  ...
  return {
    ...
    notification: { requestCode: form.notification.requestCode, requestDate: form.notification.requestDate, requestAgency: form.notification.requestAgency, appraisalResult: form.notification.appraisalResult, propertyDescription: form.notification.propertyDescription, propertyValue: form.notification.propertyValue, appraiserName: form.notification.appraiserName, appraisalDate: form.notification.appraisalDate },
...
export function Bm128FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = <T extends keyof Bm128Form>(section: T, key: keyof Bm128Form[T], value: string) => {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-128</p>
          <h2 className="mt-1 text-xl font-bold text-slate-950">Thông báo nội dung kết luận giám định, định giá tài sản</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-128.</p>
        </div>
      ...
      <SectionCard title="1. Header biểu mẫu">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Field label="Số thông báo" required value={form.document.documentCode} onChange={v => patch("document", "documentCode", v)} />
      ...
      <SectionCard title="2. Thông tin kết luận giám định/định giá" description="Nhập thông tin kết quả giám định, định giá tài sản.">
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Số yêu cầu" required value={form.notification.requestCode} onChange={v => patch("notification", "requestCode", v)} />
          <Field label="Ngày yêu cầu" value={form.notification.requestDate} onChange={v => patch("notification", "requestDate", v)} />
        </div>
        <Field label="Cơ quan thực hiện giám định/định giá" required value={form.notification.requestAgency} onChange={v => patch("notification", "requestAgency", v)} />
        <Field label="Mô tả tài sản được giám định/định giá" required value={form.notification.propertyDescription} onChange={v => patch("notification", "propertyDescription", v)} />
        <Field label="Kết quả giám định/định giá" required multiline value={form.notification.appraisalResult} onChange={v => patch("notification", "appraisalResult", v)} />
        <div className="grid gap-4 md:grid-cols-3">
          <Field label="Giá trị tài sản" value={form.notification.propertyValue} onChange={v => patch("notification", "propertyValue", v)} />
          <Field label="Người thực hiện" value={form.notification.appraiserName} onChange={v => patch("notification", "appraiserName", v)} />
          <Field label="Ngày kết luận" value={form.notification.appraisalDate} onChange={v => patch("notification", "appraisalDate", v)} />
        </div>
      </SectionCard>
