...
const EMPTY_FORM: Bm115Form = {
  ...
  document: { documentCode: "115/YCT-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
...
export function Bm115FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = <T extends keyof Bm115Form>(section: T, key: keyof Bm115Form[T], value: string) => {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-115</p>
          <h2 className="mt-1 text-xl font-bold text-slate-950">Yêu cầu phục hồi điều tra VAHS đối với bị can</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-115.</p>
        </div>
      ...
      <SectionCard title="2. Thông tin phục hồi điều tra" description="Nhập thông tin VAHS và bị can cần phục hồi điều tra.">
