...
type SearchWarrantRejectionForm = {
  procedureArticlesLine: string;
  rejectionReasonLine: string;
};
...
type Bm120Form = {
  ...
  searchWarrantRejection: SearchWarrantRejectionForm;
...
type Bm120FormInputsPanelProps = {
...
const EMPTY_FORM: Bm120Form = {
  ...
  searchWarrantRejection: {
    procedureArticlesLine: "Căn cứ Điều 36 của Bộ luật Tố tụng hình sự;",
    rejectionReasonLine: "",
  },
  recipients: {
...
function buildRejectionReasonLine(form: Bm120Form): string {
  const data = form.searchWarrantRejection;

  if (!data.rejectionReasonLine.trim()) {
    return `Xét thấy đề nghị phê chuẩn lệnh khám xét không đủ căn cứ pháp luật.`;
  }

  return data.rejectionReasonLine.trim();
}
  ...
  return {
    ...
    searchWarrantRejection: {
      procedureArticlesLine:
        nested(payload, "searchWarrantRejection.procedureArticlesLine") ||
        EMPTY_FORM.searchWarrantRejection.procedureArticlesLine,

      rejectionReasonLine:
        nested(payload, "searchWarrantRejection.rejectionReasonLine") ||
        EMPTY_FORM.searchWarrantRejection.rejectionReasonLine,
    },
    recipients: {
      archiveLine:
...
function validateForm(form: Bm120Form): string[] {
  const required = [
    ...
    ["Căn cứ tố tụng", form.searchWarrantRejection.procedureArticlesLine],
    ["Lý do không phê chuẩn", form.searchWarrantRejection.rejectionReasonLine],
...
function buildSaveBody(form: Bm120Form) {
  ...
  const searchWarrantRejection = {
    procedureArticlesLine:
      form.searchWarrantRejection.procedureArticlesLine,
    rejectionReasonLine: buildRejectionReasonLine(form),
  };
...
export function Bm120FormInputsPanel({
  ...
  const patch = <T extends keyof Bm120Form>(
    section: T,
    key: keyof Bm120Form[T],
    value: string,
  ) => {
    setForm((current) => ({
      ...current,
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải lại dữ liệu BM-120 từ backend.");
    } catch (err) {
  ...
  const handleFillSample = () => {
    setForm({
      ...
      searchWarrantRejection: {
        ...EMPTY_FORM.searchWarrantRejection,
      },
      recipients: { ...EMPTY_FORM.recipients },
      signature: { ...EMPTY_FORM.signature },
    });
    setError(null);
    setMessage("Đã điền dữ liệu mẫu BM-120.");
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu dữ liệu BM-120. Các dòng tự sinh đã đồng bộ.");
      await onSaved?.();
    } catch (err) {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <h2 className="mt-1 text-xl font-bold text-slate-950">
            QĐ không phê chuẩn Lệnh khám xét
          </h2>
      ...
      <SectionCard
        title="2. Nội dung không phê chuẩn"
      ...
      >
        <Field
          ...
          required
          multiline
          value={form.searchWarrantRejection.procedureArticlesLine}
          onChange={(value) =>
            patch("searchWarrantRejection", "procedureArticlesLine", value)
          }
        />
        ...
        <Field
          ...
          required
          multiline
          value={form.searchWarrantRejection.rejectionReasonLine}
          onChange={(value) =>
            patch("searchWarrantRejection", "rejectionReasonLine", value)
          }
        />
      </SectionCard>
      ...
      >
        <Field
          ...
          multiline
          readOnly
          value={buildRejectionReasonLine(form)}
        />
      </SectionCard>
