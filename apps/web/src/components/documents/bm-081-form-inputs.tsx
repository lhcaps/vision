...
type SectionKey = keyof Bm081FormInputs;
...
const EMPTY_FORM: Bm081FormInputs = {
  agency: {
    parentName: "",
    name: "",
    issuePlace: "",
...
function getValue(form: Bm081FormInputs, sectionKey: SectionKey, field: string): string {
  return form[sectionKey][field] ?? "";
}

function normalizeFormInputs(payload: Record<string, unknown>): Bm081FormInputs {
  ...
  return {
    ...
    document: {
      documentCode: text(document.documentCode || document.documentNo || "81/QĐ-VKSKV7"),
...
async function getBm081RenderPayload(documentId: string | number): Promise<Record<string, unknown>> {
  const response = await fetch(
    `${API_BASE_URL}/documents/generated/${documentId}/render-payload`,
    { method: "GET", headers: { Accept: "application/json" }, cache: "no-store" }
  );
  if (!response.ok) {
    throw new Error(`Không tải được payload BM-081. HTTP ${response.status}`);
  }
  return (await response.json()) as Record<string, unknown>;
}

async function saveBm081FormInputs(
  documentId: string | number,
  form: Bm081FormInputs,
): Promise<void> {
  const response = await fetch(
  ...
  if (!response.ok) {
    throw new Error(`Không lưu được BM-081. HTTP ${response.status}`);
  }
}
...
export function Bm081FormInputsPanel({ documentId, onSaved }: Bm081FormInputsPanelProps) {
  const [form, setForm] = useState<Bm081FormInputs>(EMPTY_FORM);
  ...
  function loadForm() {
    setIsLoading(true);
    setErrorMessage("");
    getBm081RenderPayload(documentId)
      .then((payload) => {
      ...
      .catch((err) => setErrorMessage(err instanceof Error ? err.message : "Không tải được BM-081"))
  ...
  function handleSave() {
    setIsSaving(true);
    setErrorMessage("");
    saveBm081FormInputs(documentId, form)
      .then(() => {
        setInitialSnapshot(currentSnapshot);
        setSavedAt(new Date());
        onSaved?.();
      })
      .catch((err) => setErrorMessage(err instanceof Error ? err.message : "Không lưu được BM-081"))
  ...
  if (isLoading) {
    return (
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <p className="text-sm text-slate-600">Đang tải dữ liệu BM-081...</p>
      </section>
    );
  }
    ...
    <div className="space-y-5">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500">BM-081</p>
            <h2 className="mt-1 text-xl font-bold text-slate-950">QĐ thời điểm người bào chữa tham gia tố tụng</h2>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
              Form nhập dữ liệu cho Quyết định về thời điểm người bào chữa tham gia tố tụng.
              Dữ liệu gồm thông tin vụ án, người bào chữa và thời điểm bắt đầu tham gia.
            </p>
          </div>
      ...
      <SectionCard title="4. Thông tin người bào chữa">
        <Field required label="Tên người bào chữa" value={form.defenseLawyer.lawyerName} onChange={(v) => updateField("defenseLawyer", "lawyerName", v)} />
        <Field label="Số GCN bào chữa" value={form.defenseLawyer.lawyerLicenseNo} onChange={(v) => updateField("defenseLawyer", "lawyerLicenseNo", v)} />
        <Field type="date" label="Ngày đăng ký bào chữa" value={form.defenseLawyer.registrationDate} onChange={(v) => updateField("defenseLawyer", "registrationDate", v)} />
      </SectionCard>
      ...
      <div className="sticky bottom-4 z-10 rounded-2xl border border-slate-200 bg-white/95 p-4 shadow-xl backdrop-blur">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          >
            {isSaving ? "Đang lưu..." : "Lưu dữ liệu BM-081"}
          </button>
        </div>
      </div>
    </div>
