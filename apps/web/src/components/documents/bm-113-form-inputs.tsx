...
const EMPTY_FORM: Bm113Form = {
  ...
  document: { documentCode: "113/YCT-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
  official: { issuerTitle: "VIỆN TRƯỞNG VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7" },
  request: { procedureArticlesLine: "Căn cứ các điều 36, 37 và 172 của Bộ luật Tố tụng hình sự;", caseTitle: "", offenseName: "", suspendedDecisionCode: "", suspendedDecisionDate: "", suspendedDecisionAgency: "", reasonLine: "" },
...
function buildReasonLine(form: Bm113Form): string {
...
function validateForm(form: Bm113Form): string[] {
  const reqs: [string, string][] = [
    ...
    ["Tội danh", form.request.offenseName], ["Số QĐ tạm đình chỉ", form.request.suspendedDecisionCode],
    ["Cơ quan ban hành QĐ", form.request.suspendedDecisionAgency],
...
function buildSaveBody(form: Bm113Form) {
  ...
  return {
    ...
    request: { procedureArticlesLine: form.request.procedureArticlesLine, caseTitle: form.request.caseTitle, offenseName: form.request.offenseName, suspendedDecisionCode: form.request.suspendedDecisionCode, suspendedDecisionDate: form.request.suspendedDecisionDate, suspendedDecisionAgency: form.request.suspendedDecisionAgency, reasonLine: buildReasonLine(form) },
...
export function Bm113FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = <T extends keyof Bm113Form>(section: T, key: keyof Bm113Form[T], value: string) => {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-113</p>
          <h2 className="mt-1 text-xl font-bold text-slate-950">Yêu cầu phục hồi điều tra VAHS</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-113.</p>
        </div>
