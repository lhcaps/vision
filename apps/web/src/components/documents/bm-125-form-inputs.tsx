...
const EMPTY_FORM: Bm125Form = {
  ...
  document: { documentCode: "125/TB-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
  official: { issuerTitle: "VIỆN TRƯỞNG VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7" },
  notification: { requestedAgency: "", requestCode: "", requestDate: "", requestContent: "", refusalReason: "" },
...
function validateForm(form: Bm125Form): string[] {
  const reqs: [string, string][] = [
    ...
    ["Cơ quan yêu cầu", form.notification.requestedAgency], ["Số yêu cầu", form.notification.requestCode],
    ["Nội dung yêu cầu", form.notification.requestContent], ["Lý do không chấp nhận", form.notification.refusalReason],
...
function buildSaveBody(form: Bm125Form) {
  ...
  return {
    ...
    notification: { requestedAgency: form.notification.requestedAgency, requestCode: form.notification.requestCode, requestDate: form.notification.requestDate, requestContent: form.notification.requestContent, refusalReason: form.notification.refusalReason },
...
export function Bm125FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = <T extends keyof Bm125Form>(section: T, key: keyof Bm125Form[T], value: string) => {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-125</p>
          <h2 className="mt-1 text-xl font-bold text-slate-950">Thông báo về việc không chấp nhận đề nghị trưng cầu giám định, định giá tài sản</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-125.</p>
        </div>
      ...
      <SectionCard title="2. Thông tin không chấp nhận" description="Nhập thông tin đề nghị không được chấp nhận.">
        <Field label="Cơ quan đề nghị" required value={form.notification.requestedAgency} onChange={v => patch("notification", "requestedAgency", v)} />
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Số đề nghị" value={form.notification.requestCode} onChange={v => patch("notification", "requestCode", v)} />
          <Field label="Ngày đề nghị" value={form.notification.requestDate} onChange={v => patch("notification", "requestDate", v)} />
        </div>
        <Field label="Nội dung đề nghị" required multiline value={form.notification.requestContent} onChange={v => patch("notification", "requestContent", v)} />
        <Field label="Lý do không chấp nhận" required multiline value={form.notification.refusalReason} onChange={v => patch("notification", "refusalReason", v)} />
      </SectionCard>
