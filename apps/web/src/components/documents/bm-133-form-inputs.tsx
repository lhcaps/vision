...
type Bm133Form = {
  ...
  decision: { procedureArticlesLine: string; caseTitle: string; offenseName: string; expertName: string; originalAppraisalCode: string; originalAppraisalDate: string; originalAppraisalAgency: string; specialReason: string; reasonLine: string };
...
const EMPTY_FORM: Bm133Form = {
  ...
  decision: { procedureArticlesLine: "Căn cứ các điều 36, 37 và 51 của Bộ luật Tố tụng hình sự;", caseTitle: "", offenseName: "", expertName: "", originalAppraisalCode: "", originalAppraisalDate: "", originalAppraisalAgency: "", specialReason: "", reasonLine: "" },
...
function buildIssuePlaceAndDateLine(form: Bm133Form): string { const place = form.document.issuePlace.trim(); const dateText = toVietnameseDateText(form.document.issueDateIso); return place ? place + ", " + dateText : dateText; }
function buildReasonLine(form: Bm133Form): string { const d = form.decision; if (!d.caseTitle) return ""; return "Qua xét nội dung vụ án " + d.caseTitle.trim() + (d.offenseName ? " về tội \"" + d.offenseName.trim() + "\"" : "") + ", căn cứ kết luận giám định số " + d.originalAppraisalCode.trim() + (d.originalAppraisalDate ? " ngày " + cleanText(d.originalAppraisalDate) : "") + " của " + (d.expertName ? "giám định viên " + d.expertName.trim() : d.originalAppraisalAgency.trim()) + ". Trong trường hợp đặc biệt, cần giám định lại vì " + d.specialReason.trim() + "."; }

function normalizeFormInputs(payload: any): Bm133Form {
  const f = EMPTY_FORM;
  return {
    ...
    decision: { procedureArticlesLine: nested(payload, "decision.procedureArticlesLine") || f.decision.procedureArticlesLine, caseTitle: nested(payload, "decision.caseTitle") || nested(payload, "case.caseTitle") || "", offenseName: nested(payload, "decision.offenseName") || nested(payload, "offense.offenseName") || "", expertName: nested(payload, "decision.expertName") || "", originalAppraisalCode: nested(payload, "decision.originalAppraisalCode") || "", originalAppraisalDate: nested(payload, "decision.originalAppraisalDate") || "", originalAppraisalAgency: nested(payload, "decision.originalAppraisalAgency") || "", specialReason: nested(payload, "decision.specialReason") || "", reasonLine: nested(payload, "decision.reasonLine") || "" },
...
function validateForm(form: Bm133Form): string[] {
  const reqs: [string, string][] = [["Viện kiểm sát cấp trên", form.agency.parentName], ["Viện kiểm sát ban hành", form.agency.name], ["Số quyết định", form.document.documentCode], ["Địa danh", form.document.issuePlace], ["Ngày ban hành", form.document.issueDateIso], ["Chủ thể ban hành", form.official.issuerTitle], ["Căn cứ pháp luật", form.decision.procedureArticlesLine], ["Tên vụ án", form.decision.caseTitle], ["Số giám định gốc", form.decision.originalAppraisalCode], ["Lý do đặc biệt", form.decision.specialReason], ["Chế độ ký", form.signature.signMode], ["Chức vụ ký", form.signature.positionTitle], ["Người ký", form.signature.signerName]];
...
function buildSaveBody(form: Bm133Form) {
  return {
    ...
    decision: { procedureArticlesLine: form.decision.procedureArticlesLine, caseTitle: form.decision.caseTitle, offenseName: form.decision.offenseName, expertName: form.decision.expertName, originalAppraisalCode: form.decision.originalAppraisalCode, originalAppraisalDate: form.decision.originalAppraisalDate, originalAppraisalAgency: form.decision.originalAppraisalAgency, specialReason: form.decision.specialReason, reasonLine: buildReasonLine(form) },
...
export function Bm133FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = (section: keyof Bm133Form, key: string, value: string) => setForm((f) => ({ ...f, [section]: { ...(f[section] as Record<string, string>), [key]: value } }));
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div><p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-133</p><h2 className="mt-1 text-xl font-bold text-slate-950">QĐ giám định lại trong trường hợp đặc biệt</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-133.</p></div>
      ...
      <SectionCard title="2. Thông tin giám định lại đặc biệt" description="Nhập thông tin giám định lại trong trường hợp đặc biệt.">
        ...
        <Field label="Tên giám định viên" value={form.decision.expertName} onChange={(v) => patch("decision", "expertName", v)} />
        <div className="grid gap-4 md:grid-cols-3">
          <Field label="Số giám định gốc" required value={form.decision.originalAppraisalCode} onChange={(v) => patch("decision", "originalAppraisalCode", v)} />
          <Field label="Ngày giám định gốc" value={form.decision.originalAppraisalDate} onChange={(v) => patch("decision", "originalAppraisalDate", v)} />
          <Field label="Đơn vị giám định gốc" value={form.decision.originalAppraisalAgency} onChange={(v) => patch("decision", "originalAppraisalAgency", v)} />
        </div>
        <Field label="Lý do đặc biệt cần giám định lại" required multiline value={form.decision.specialReason} onChange={(v) => patch("decision", "specialReason", v)} />
