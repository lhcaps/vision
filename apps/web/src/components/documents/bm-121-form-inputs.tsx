...
const EMPTY_FORM: Bm121Form = {
  ...
  decision: { procedureArticlesLine: "Căn cứ các điều 36, 37 và 116 của Bộ luật Tố tụng hình sự;", caseTitle: "", orderCode: "", orderDate: "", orderAgency: "", propertyDescription: "", reasonLine: "" },
...
function buildReasonLine(form: Bm121Form): string {
  const d = form.decision;
  if (!d.caseTitle) return "";
  return `Qua xét nội dung vụ án ${d.caseTitle.trim()}, căn cứ Lệnh số ${d.orderCode.trim()}${d.orderDate ? ` ngày ${cleanText(d.orderDate)}` : ""} của ${d.orderAgency.trim()}${d.propertyDescription ? ` về việc thu giữ ${d.propertyDescription.trim()}` : ""}. Có căn cứ cho rằng việc thu giữ thư tín, điện tín, bưu kiện, bưu phẩm là có căn cứ và đúng pháp luật.`;
}
...
function validateForm(form: Bm121Form): string[] {
  const reqs: [string, string][] = [
    ...
    ["Căn cứ pháp luật", form.decision.procedureArticlesLine], ["Tên vụ án", form.decision.caseTitle],
    ["Số Lệnh", form.decision.orderCode], ["Cơ quan ban hành Lệnh", form.decision.orderAgency],
...
function buildSaveBody(form: Bm121Form) {
  ...
  return {
    ...
    decision: { procedureArticlesLine: form.decision.procedureArticlesLine, caseTitle: form.decision.caseTitle, orderCode: form.decision.orderCode, orderDate: form.decision.orderDate, orderAgency: form.decision.orderAgency, propertyDescription: form.decision.propertyDescription, reasonLine: buildReasonLine(form) },
...
export function Bm121FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = <T extends keyof Bm121Form>(section: T, key: keyof Bm121Form[T], value: string) => {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-121</p>
          <h2 className="mt-1 text-xl font-bold text-slate-950">QĐ phê chuẩn Lệnh thu giữ thư tín, điện tín, bưu kiện, bưu phẩm</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-121.</p>
        </div>
      ...
      <SectionCard title="2. Thông tin phê chuẩn Lệnh" description="Nhập thông tin Lệnh thu giữ cần phê chuẩn.">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Số Lệnh" required value={form.decision.orderCode} onChange={v => patch("decision", "orderCode", v)} />
          <Field label="Ngày Lệnh" value={form.decision.orderDate} onChange={v => patch("decision", "orderDate", v)} />
        </div>
        <Field label="Cơ quan ban hành Lệnh" required value={form.decision.orderAgency} onChange={v => patch("decision", "orderAgency", v)} />
        <Field label="Mô tả đối tượng thu giữ" value={form.decision.propertyDescription} onChange={v => patch("decision", "propertyDescription", v)} />
        <Field label="Lý do phê chuẩn tự sinh" multiline readOnly value={buildReasonLine(form)} />
      </SectionCard>
