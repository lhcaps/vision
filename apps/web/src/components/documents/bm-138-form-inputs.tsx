...
const EMPTY_FORM: Bm138Form = {
  ...
  document: { documentCode: "138/YCT-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
  official: { issuerTitle: "VIỆN TRƯỞNG VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7" },
  request: { procedureArticlesLine: "Căn cứ các điều 36, 37 và 85 của Bộ luật Tố tụng hình sự;", caseTitle: "", offenseName: "", requestedAgency: "", requestedAgencyAddress: "", documentDescription: "", illegalContent: "", requestContent: "", reasonLine: "" },
...
function buildReasonLine(form: Bm138Form): string {
  const r = form.request;
  if (!r.caseTitle) return "";
  return `Trong quá trình điều tra vụ án ${r.caseTitle.trim()} về tội "${r.offenseName.trim()}", có căn cứ cho rằng ${r.requestedAgency.trim()}${r.requestedAgencyAddress ? ` (địa chỉ: ${r.requestedAgencyAddress.trim()})` : ""} có tài liệu ${r.documentDescription.trim()} liên quan đến hành vi phạm tội và QĐ tố tụng có vi phạm pháp luật. Đề nghị cung cấp ${r.requestContent.trim()}.`;
}
...
function normalizeFormInputs(payload: RenderPayload | null): Bm138Form {
  ...
  return {
    ...
    request: {
      ...
      requestedAgencyAddress: nested(payload, "request.requestedAgencyAddress") || "",
      documentDescription: nested(payload, "request.documentDescription") || "",
      illegalContent: nested(payload, "request.illegalContent") || "",
...
function validateForm(form: Bm138Form): string[] {
  const reqs: [string, string][] = [
    ...
    ["Tội danh", form.request.offenseName], ["Cơ quan được yêu cầu", form.request.requestedAgency],
    ["Nội dung yêu cầu", form.request.requestContent],
...
function buildSaveBody(form: Bm138Form) {
  ...
  return {
    ...
    request: { procedureArticlesLine: form.request.procedureArticlesLine, caseTitle: form.request.caseTitle, offenseName: form.request.offenseName, requestedAgency: form.request.requestedAgency, requestedAgencyAddress: form.request.requestedAgencyAddress, documentDescription: form.request.documentDescription, illegalContent: form.request.illegalContent, requestContent: form.request.requestContent, reasonLine: buildReasonLine(form) },
...
export function Bm138FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = <T extends keyof Bm138Form>(section: T, key: keyof Bm138Form[T], value: string) => {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-138</p>
          <h2 className="mt-1 text-xl font-bold text-slate-950">Yêu cầu cung cấp tài liệu liên quan đến hành vi, QĐ tố tụng có vi phạm pháp luật trong điều tra</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-138.</p>
        </div>
      ...
      <SectionCard title="2. Nội dung yêu cầu cung cấp tài liệu" description="Nhập thông tin tài liệu cần yêu cầu cung cấp.">
        ...
        <Field label="Tội danh" required value={form.request.offenseName} onChange={v => patch("request", "offenseName", v)} />
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Cơ quan được yêu cầu" required value={form.request.requestedAgency} onChange={v => patch("request", "requestedAgency", v)} />
          <Field label="Địa chỉ cơ quan" value={form.request.requestedAgencyAddress} onChange={v => patch("request", "requestedAgencyAddress", v)} />
        </div>
        <Field label="Mô tả tài liệu cần cung cấp" value={form.request.documentDescription} onChange={v => patch("request", "documentDescription", v)} />
        <Field label="Nội dung vi phạm pháp luật (nếu có)" value={form.request.illegalContent} onChange={v => patch("request", "illegalContent", v)} />
        <Field label="Nội dung yêu cầu" required value={form.request.requestContent} onChange={v => patch("request", "requestContent", v)} />
        <Field label="Lý do tự sinh" multiline readOnly value={buildReasonLine(form)} />
      </SectionCard>
