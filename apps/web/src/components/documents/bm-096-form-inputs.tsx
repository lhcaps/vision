...
const EMPTY_FORM: Bm096Form = {
  ...
  request: { procedureArticlesLine: "Căn cứ các điều 36, 37 và 167 của Bộ luật Tố tụng hình sự;", caseTitle: "", offenseName: "", accusedFullName: "", accusedDateOfBirth: "", accusedAddress: "", reasonLine: "" },
...
function normalizeFormInputs(payload: RenderPayload | null): Bm096Form {
  ...
  return {
    ...
    request: {
      ...
      accusedFullName: nested(payload, "request.accusedFullName") || nested(payload, "accused.fullName") || nested(payload, "suspect.fullName") || "",
...
function buildSaveBody(form: Bm096Form) {
  ...
  return {
    ...
    request: { procedureArticlesLine: form.request.procedureArticlesLine, caseTitle: form.request.caseTitle, offenseName: form.request.offenseName, accusedFullName: form.request.accusedFullName, accusedDateOfBirth: form.request.accusedDateOfBirth, accusedAddress: form.request.accusedAddress, reasonLine: buildReasonLine(form) },
...
export function Bm096FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = <T extends keyof Bm096Form>(section: T, key: keyof Bm096Form[T], value: string) => {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-096</p>
          <h2 className="mt-1 text-xl font-bold text-slate-950">Yêu cầu ra QĐ khởi tố bị can</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-096.</p>
        </div>
      ...
      <SectionCard title="2. Nội dung yêu cầu" description="Nhập thông tin vụ án và bị can cần khởi tố.">
