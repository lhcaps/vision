...
type Bm102FormInputsPanelProps = {
...
const EMPTY_FORM: Bm102Form = {
  ...
  decisionCancel: {
    procedureArticlesLine:
      "Căn cứ các điều 36, 176, 177, 178 của Bộ luật Tố tụng hình sự;",
...
function buildReasonLine(form: Bm102Form): string {
  ...
  if (!data.reasonLine.trim()) {
    return `Xét thấy việc hủy bỏ quyết định khởi tố bị can là cần thiết và có căn cứ.`;
  }
...
function validateForm(form: Bm102Form): string[] {
  const required = [
    ...
    ["Căn cứ tố tụng", form.decisionCancel.procedureArticlesLine],
    ["Quyết định bị hủy", form.decisionCancel.decisionInfoLine],
    ["Lý do hủy bỏ", form.decisionCancel.reasonLine],
    ["Tên bị can", form.decisionCancel.personName],
    ["Tội danh", form.decisionCancel.offenseName],
...
function buildSaveBody(form: Bm102Form) {
  ...
  const decisionCancel = {
    procedureArticlesLine:
      form.decisionCancel.procedureArticlesLine,
...
export function Bm102FormInputsPanel({
  ...
  const patch = <T extends keyof Bm102Form>(
    section: T,
    key: keyof Bm102Form[T],
    value: string,
  ) => {
    setForm((current) => ({
      ...current,
    ...
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      ...
      >
        <Field
          ...
          onChange={(value) =>
            patch("decisionCancel", "procedureArticlesLine", value)
          }
        />

        <Field
          label="Quyết định khởi tố bị can bị hủy"
          required
          value={form.decisionCancel.decisionInfoLine}
          onChange={(value) =>
            patch("decisionCancel", "decisionInfoLine", value)
          }
        />
        ...
        <Field
          ...
          onChange={(value) =>
            patch("decisionCancel", "reasonLine", value)
          }
        />
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Field
            ...
            onChange={(value) =>
              patch("decisionCancel", "personName", value)
            }
          />
          ...
          <Field
            ...
            onChange={(value) =>
              patch("decisionCancel", "offenseName", value)
            }
          />
        </div>
      </SectionCard>
