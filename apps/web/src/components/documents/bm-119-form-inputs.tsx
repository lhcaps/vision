...
type SearchWarrantApprovalForm = {
  procedureArticlesLine: string;
  searchReasonLine: string;
  locationLine: string;
  searchContentLine: string;
};
...
type Bm119Form = {
  ...
  searchWarrantApproval: SearchWarrantApprovalForm;
...
type Bm119FormInputsPanelProps = {
...
const EMPTY_FORM: Bm119Form = {
  ...
  searchWarrantApproval: {
    procedureArticlesLine:
      "Căn cứ các điều 36, 195, 196 của Bộ luật Tố tụng hình sự;",
    searchReasonLine: "",
    locationLine: "",
    searchContentLine: "",
  },
...
function buildSearchContentLine(form: Bm119Form): string {
  const data = form.searchWarrantApproval;

  if (!data.searchContentLine.trim()) {
    return `Khám xét ${data.locationLine.trim()} để thu thập tài liệu, đồ vật liên quan đến vụ án.`;
  }

  return data.searchContentLine.trim();
}
...
function normalizeFormInputs(payload: RenderPayload | null): Bm119Form {
  ...
  return {
    ...
    searchWarrantApproval: {
      procedureArticlesLine:
        nested(payload, "searchWarrantApproval.procedureArticlesLine") ||
        EMPTY_FORM.searchWarrantApproval.procedureArticlesLine,

      searchReasonLine:
        nested(payload, "searchWarrantApproval.searchReasonLine") ||
        EMPTY_FORM.searchWarrantApproval.searchReasonLine,

      locationLine:
        nested(payload, "searchWarrantApproval.locationLine") ||
        EMPTY_FORM.searchWarrantApproval.locationLine,

      searchContentLine:
        nested(payload, "searchWarrantApproval.searchContentLine") ||
        EMPTY_FORM.searchWarrantApproval.searchContentLine,
    },
    recipients: {
      archiveLine:
...
function validateForm(form: Bm119Form): string[] {
  const required = [
    ...
    ["Căn cứ tố tụng", form.searchWarrantApproval.procedureArticlesLine],
    ["Lý do khám xét", form.searchWarrantApproval.searchReasonLine],
    ["Địa điểm khám xét", form.searchWarrantApproval.locationLine],
...
function buildSaveBody(form: Bm119Form) {
  ...
  const searchWarrantApproval = {
    procedureArticlesLine:
      form.searchWarrantApproval.procedureArticlesLine,
    searchReasonLine: form.searchWarrantApproval.searchReasonLine,
    locationLine: form.searchWarrantApproval.locationLine,
    searchContentLine: buildSearchContentLine(form),
  };
...
export function Bm119FormInputsPanel({
  ...
  const patch = <T extends keyof Bm119Form>(
    section: T,
    key: keyof Bm119Form[T],
    value: string,
  ) => {
    setForm((current) => ({
      ...current,
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải lại dữ liệu BM-119 từ backend.");
    } catch (err) {
  ...
  const handleFillSample = () => {
    setForm({
      ...
      searchWarrantApproval: {
        ...EMPTY_FORM.searchWarrantApproval,
      },
      recipients: { ...EMPTY_FORM.recipients },
      signature: { ...EMPTY_FORM.signature },
    });
    setError(null);
    setMessage("Đã điền dữ liệu mẫu BM-119.");
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu dữ liệu BM-119. Các dòng tự sinh đã đồng bộ.");
      await onSaved?.();
    } catch (err) {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      ...
      <SectionCard
        title="2. Nội dung phê chuẩn lệnh khám xét"
      ...
      >
        <Field
          ...
          required
          multiline
          value={form.searchWarrantApproval.procedureArticlesLine}
          onChange={(value) =>
            patch("searchWarrantApproval", "procedureArticlesLine", value)
          }
        />

        <Field
          label="Lý do khám xét"
          required
          multiline
          value={form.searchWarrantApproval.searchReasonLine}
          onChange={(value) =>
            patch("searchWarrantApproval", "searchReasonLine", value)
          }
        />
        ...
        <Field
          ...
          required
          value={form.searchWarrantApproval.locationLine}
          onChange={(value) =>
            patch("searchWarrantApproval", "locationLine", value)
          }
        />

        <Field
          label="Nội dung khám xét (tự sinh nếu trống)"
          multiline
          value={form.searchWarrantApproval.searchContentLine}
          onChange={(value) =>
            patch("searchWarrantApproval", "searchContentLine", value)
          }
        />
      </SectionCard>
      ...
      >
        <Field
          label="Dòng nội dung khám xét tự sinh"
          multiline
          readOnly
          value={buildSearchContentLine(form)}
        />
      </SectionCard>
