...
const EMPTY_FORM: Bm129Form = {
  ...
  decision: { procedureArticlesLine: "Căn cứ các điều 36, 37 và 51 của Bộ luật Tố tụng hình sự;", caseTitle: "", offenseName: "", originalAppraisalCode: "", originalAppraisalDate: "", originalAppraisalAgency: "", supplementReason: "", reasonLine: "" },
...
function validateForm(form: Bm129Form): string[] {
  const reqs: [string, string][] = [
    ...
    ["Số giám định gốc", form.decision.originalAppraisalCode], ["Lý do giám định bổ sung", form.decision.supplementReason],
...
export function Bm129FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = <T extends keyof Bm129Form>(section: T, key: keyof Bm129Form[T], value: string) => {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-129</p>
          <h2 className="mt-1 text-xl font-bold text-slate-950">QĐ trưng cầu giám định bổ sung</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-129.</p>
        </div>
      ...
      <SectionCard title="2. Thông tin trưng cầu giám định bổ sung" description="Nhập thông tin giám định bổ sung.">
        ...
        <Field label="Lý do giám định bổ sung" required multiline value={form.decision.supplementReason} onChange={v => patch("decision", "supplementReason", v)} />
