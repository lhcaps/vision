...
type Bm130Form = {
  ...
  decision: {
    procedureArticlesLine: string; caseTitle: string; offenseName: string;
    originalAppraisalCode: string; originalAppraisalDate: string; originalAppraisalAgency: string;
    reappraisalReason: string; reasonLine: string;
  };
...
const EMPTY_FORM: Bm130Form = {
  ...
  decision: { procedureArticlesLine: "Căn cứ các điều 36, 37 và 51 của Bộ luật Tố tụng hình sự;", caseTitle: "", offenseName: "", originalAppraisalCode: "", originalAppraisalDate: "", originalAppraisalAgency: "", reappraisalReason: "", reasonLine: "" },
...
function buildIssuePlaceAndDateLine(form: Bm130Form): string { const place = form.document.issuePlace.trim(); const dateText = toVietnameseDateText(form.document.issueDateIso); return place ? place + ", " + dateText : dateText; }
function buildReasonLine(form: Bm130Form): string { const d = form.decision; if (!d.caseTitle) return ""; return "Qua xét nội dung vụ án " + d.caseTitle.trim() + (d.offenseName ? " về tội \"" + d.offenseName.trim() + "\"" : "") + ", căn cứ kết luận giám định số " + d.originalAppraisalCode.trim() + (d.originalAppraisalDate ? " ngày " + cleanText(d.originalAppraisalDate) : "") + " của " + d.originalAppraisalAgency.trim() + ". Có căn cứ cho rằng cần giám định lại vì " + d.reappraisalReason.trim() + "."; }

function normalizeFormInputs(payload: any): Bm130Form {
  const f = EMPTY_FORM;
  return {
    ...
    decision: { procedureArticlesLine: nested(payload, "decision.procedureArticlesLine") || f.decision.procedureArticlesLine, caseTitle: nested(payload, "decision.caseTitle") || nested(payload, "case.caseTitle") || "", offenseName: nested(payload, "decision.offenseName") || nested(payload, "offense.offenseName") || "", originalAppraisalCode: nested(payload, "decision.originalAppraisalCode") || "", originalAppraisalDate: nested(payload, "decision.originalAppraisalDate") || "", originalAppraisalAgency: nested(payload, "decision.originalAppraisalAgency") || "", reappraisalReason: nested(payload, "decision.reappraisalReason") || "", reasonLine: nested(payload, "decision.reasonLine") || "" },
...
function buildSaveBody(form: Bm130Form) {
  return {
    ...
    decision: { procedureArticlesLine: form.decision.procedureArticlesLine, caseTitle: form.decision.caseTitle, offenseName: form.decision.offenseName, originalAppraisalCode: form.decision.originalAppraisalCode, originalAppraisalDate: form.decision.originalAppraisalDate, originalAppraisalAgency: form.decision.originalAppraisalAgency, reappraisalReason: form.decision.reappraisalReason, reasonLine: buildReasonLine(form) },
...
export function Bm130FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = (section: keyof Bm130Form, key: string, value: string) => setForm((f) => ({ ...f, [section]: { ...(f[section] as Record<string, string>), [key]: value } }));
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div><p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-130</p><h2 className="mt-1 text-xl font-bold text-slate-950">QĐ trưng cầu giám định lại</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-130.</p></div>
      ...
      <SectionCard title="2. Thông tin trưng cầu giám định lại" description="Nhập thông tin giám định lại.">
        ...
        <Field label="Lý do giám định lại" required multiline value={form.decision.reappraisalReason} onChange={(v) => patch("decision", "reappraisalReason", v)} />
