...
function buildReasonLine(form: Bm109Form): string {
  ...
  return `Xét thấy QĐ số ${d.cancelledDocumentCode.trim()}${dateText ? ` ${dateText}` : ""} của ${d.cancelledDocumentAgency.trim()} về việc tạm đình chỉ điều tra VAHS đối với bị can ${d.accusedFullName.trim()} trong vụ án ${d.caseTitle.trim()} về tội "${d.offenseName.trim()}" không có căn cứ và trái pháp luật;`;
}
...
export function Bm109FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = <T extends keyof Bm109Form>(section: T, key: keyof Bm109Form[T], value: string) => {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-109</p>
          <h2 className="mt-1 text-xl font-bold text-slate-950">QĐ huỷ bỏ QĐ tạm đình chỉ điều tra VAHS đối với bị can</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-109.</p>
        </div>
      ...
      <SectionCard title="2. Quyết định bị huỷ bỏ" description="Nhập thông tin QĐ tạm đình chỉ VAHS cần huỷ bỏ.">
