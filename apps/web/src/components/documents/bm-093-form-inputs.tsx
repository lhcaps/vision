...
function normalizeFormInputs(payload: RenderPayload | null): Bm093Form {
  ...
  return {
    ...
    cancelledDecision: {
      ...
      accusedFullName: nested(payload, "cancelledDecision.accusedFullName") || nested(payload, "accused.fullName") || nested(payload, "suspect.fullName") || "",
...
export function Bm093FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = <T extends keyof Bm093Form>(section: T, key: keyof Bm093Form[T], value: string) => {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-093</p>
          <h2 className="mt-1 text-xl font-bold text-slate-950">QĐ huỷ bỏ QĐ thay đổi QĐ khởi tố bị can</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-093.</p>
        </div>
      ...
      <SectionCard title="2. Quyết định bị huỷ bỏ" description="Nhập thông tin QĐ thay đổi khởi tố bị can cần huỷ bỏ.">
