...
const EMPTY_FORM: Bm127Form = {
  ...
  document: { documentCode: "127/YCT-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
  official: { issuerTitle: "VIỆN TRƯỞNG VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7" },
  request: { procedureArticlesLine: "Căn cứ các điều 36, 37 và 50 của Bộ luật Tố tụng hình sự;", caseTitle: "", propertyDescription: "", propertyValue: "", appraisalAgency: "", reasonLine: "" },
...
function buildReasonLine(form: Bm127Form): string {
  const r = form.request;
  if (!r.propertyDescription || !r.caseTitle) return "";
  return `Qua xét nội dung vụ án ${r.caseTitle.trim()}, để phục vụ yêu cầu điều tra, cần xác định giá trị tài sản ${r.propertyDescription.trim()}${r.propertyValue ? ` (ước tính: ${r.propertyValue.trim()})` : ""}. Đề nghị ${r.appraisalAgency.trim()} tiến hành định giá tài sản theo quy định.`;
}
...
function normalizeFormInputs(payload: RenderPayload | null): Bm127Form {
  ...
  return {
    ...
    request: {
      ...
      propertyValue: nested(payload, "request.propertyValue") || "",
      appraisalAgency: nested(payload, "request.appraisalAgency") || "",
...
function validateForm(form: Bm127Form): string[] {
  const reqs: [string, string][] = [
    ...
    ["Mô tả tài sản", form.request.propertyDescription], ["Đơn vị định giá", form.request.appraisalAgency],
...
function buildSaveBody(form: Bm127Form) {
  ...
  return {
    ...
    request: { procedureArticlesLine: form.request.procedureArticlesLine, caseTitle: form.request.caseTitle, propertyDescription: form.request.propertyDescription, propertyValue: form.request.propertyValue, appraisalAgency: form.request.appraisalAgency, reasonLine: buildReasonLine(form) },
...
export function Bm127FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = <T extends keyof Bm127Form>(section: T, key: keyof Bm127Form[T], value: string) => {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-127</p>
          <h2 className="mt-1 text-xl font-bold text-slate-950">Yêu cầu định giá tài sản</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-127.</p>
        </div>
      ...
      <SectionCard title="2. Thông tin định giá tài sản" description="Nhập thông tin tài sản cần định giá.">
        ...
        <Field label="Mô tả tài sản cần định giá" required value={form.request.propertyDescription} onChange={v => patch("request", "propertyDescription", v)} />
        <Field label="Giá trị ước tính (nếu có)" value={form.request.propertyValue} onChange={v => patch("request", "propertyValue", v)} />
        <Field label="Đơn vị thực hiện định giá" required value={form.request.appraisalAgency} onChange={v => patch("request", "appraisalAgency", v)} />
