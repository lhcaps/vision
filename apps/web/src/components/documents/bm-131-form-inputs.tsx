...
const EMPTY_FORM: Bm131Form = {
  ...
  document: { documentCode: "131/YCT-VKSKV7", issuePlace: "TP. Hồ Chí Minh", issueDateIso: "" },
  official: { issuerTitle: "VIỆN TRƯỞNG VIỆN KIỂM SÁT NHÂN DÂN KHU VỰC 7" },
  request: { procedureArticlesLine: "Căn cứ các điều 36, 37 và 50 của Bộ luật Tố tụng hình sự;", caseTitle: "", propertyDescription: "", originalAppraisalValue: "", originalAppraisalDate: "", originalAppraisalAgency: "", reasonForReappraisal: "", reasonLine: "" },
...
function validateForm(form: Bm131Form): string[] {
  const reqs: [string, string][] = [
    ...
    ["Mô tả tài sản", form.request.propertyDescription], ["Lý do định giá lại", form.request.reasonForReappraisal],
...
function buildSaveBody(form: Bm131Form) {
  ...
  return {
    ...
    request: { procedureArticlesLine: form.request.procedureArticlesLine, caseTitle: form.request.caseTitle, propertyDescription: form.request.propertyDescription, originalAppraisalValue: form.request.originalAppraisalValue, originalAppraisalDate: form.request.originalAppraisalDate, originalAppraisalAgency: form.request.originalAppraisalAgency, reasonForReappraisal: form.request.reasonForReappraisal, reasonLine: buildReasonLine(form) },
...
export function Bm131FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = <T extends keyof Bm131Form>(section: T, key: keyof Bm131Form[T], value: string) => {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-131</p>
          <h2 className="mt-1 text-xl font-bold text-slate-950">Yêu cầu định giá lại tài sản</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-131.</p>
        </div>
      ...
      <SectionCard title="2. Thông tin định giá lại" description="Nhập thông tin định giá lại tài sản.">
        ...
        <Field label="Mô tả tài sản cần định giá lại" required value={form.request.propertyDescription} onChange={v => patch("request", "propertyDescription", v)} />
        <div className="grid gap-4 md:grid-cols-3">
          <Field label="Giá trị định giá trước" value={form.request.originalAppraisalValue} onChange={v => patch("request", "originalAppraisalValue", v)} />
          <Field label="Ngày định giá trước" value={form.request.originalAppraisalDate} onChange={v => patch("request", "originalAppraisalDate", v)} />
          <Field label="Đơn vị định giá trước" value={form.request.originalAppraisalAgency} onChange={v => patch("request", "originalAppraisalAgency", v)} />
        </div>
        <Field label="Lý do định giá lại" required value={form.request.reasonForReappraisal} onChange={v => patch("request", "reasonForReappraisal", v)} />
