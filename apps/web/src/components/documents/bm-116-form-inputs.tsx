...
type Bm116Form = {
  ...
  investigationRestore: InvestigationRestoreForm;
...
type Bm116FormInputsPanelProps = {
...
function buildReasonLine(form: Bm116Form): string {
  const data = form.investigationRestore;

  if (!data.reasonLine.trim()) {
    return `Xét thấy việc phục hồi điều tra vụ án hình sự là có căn cứ và cần thiết.`;
  }
...
function normalizeFormInputs(payload: RenderPayload | null): Bm116Form {
  ...
  return {
    ...
    investigationRestore: {
      procedureArticlesLine:
        nested(payload, "investigationRestore.procedureArticlesLine") ||
        EMPTY_FORM.investigationRestore.procedureArticlesLine,

      suspendedDecisionInfoLine:
        nested(payload, "investigationRestore.suspendedDecisionInfoLine") ||
        EMPTY_FORM.investigationRestore.suspendedDecisionInfoLine,

      caseTitle:
        nested(payload, "investigationRestore.caseTitle") ||
        nested(payload, "case.caseTitle") ||
        EMPTY_FORM.investigationRestore.caseTitle,

      offenseName:
        nested(payload, "investigationRestore.offenseName") ||
        nested(payload, "offense.offenseName") ||
        EMPTY_FORM.investigationRestore.offenseName,

      reasonLine:
        nested(payload, "investigationRestore.reasonLine") ||
        EMPTY_FORM.investigationRestore.reasonLine,
    },
    recipients: {
      archiveLine:
...
function validateForm(form: Bm116Form): string[] {
  const required = [
    ...
    ["Căn cứ tố tụng", form.investigationRestore.procedureArticlesLine],
    ["QĐ tạm đình chỉ", form.investigationRestore.suspendedDecisionInfoLine],
    ["Tên vụ án", form.investigationRestore.caseTitle],
    ["Tội danh", form.investigationRestore.offenseName],
    ["Lý do phục hồi", form.investigationRestore.reasonLine],
...
function buildSaveBody(form: Bm116Form) {
  ...
  const investigationRestore = {
    procedureArticlesLine:
      form.investigationRestore.procedureArticlesLine,
    suspendedDecisionInfoLine:
      form.investigationRestore.suspendedDecisionInfoLine,
    caseTitle: form.investigationRestore.caseTitle,
    offenseName: form.investigationRestore.offenseName,
...
export function Bm116FormInputsPanel({
  ...
  const patch = <T extends keyof Bm116Form>(
    section: T,
    key: keyof Bm116Form[T],
    value: string,
  ) => {
    setForm((current) => ({
      ...current,
  ...
  const reloadFromBackend = async () => {
    ...
    try {
      ...
      setMessage("Đã tải lại dữ liệu BM-116 từ backend.");
    } catch (err) {
  ...
  const handleFillSample = () => {
    setForm({
      ...
      investigationRestore: {
        ...EMPTY_FORM.investigationRestore,
      },
      recipients: { ...EMPTY_FORM.recipients },
      signature: { ...EMPTY_FORM.signature },
    });
    setError(null);
    setMessage("Đã điền dữ liệu mẫu BM-116.");
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setMessage("Đã lưu dữ liệu BM-116. Các dòng tự sinh đã đồng bộ.");
      await onSaved?.();
    } catch (err) {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <h2 className="mt-1 text-xl font-bold text-slate-950">
            QĐ phục hồi điều tra vụ án hình sự
          </h2>
      ...
      <SectionCard
        title="2. Nội dung phục hồi điều tra"
      ...
      >
        <Field
          ...
          required
          multiline
          value={form.investigationRestore.procedureArticlesLine}
          onChange={(value) =>
            patch("investigationRestore", "procedureArticlesLine", value)
          }
        />

        <Field
          label="QĐ tạm đình chỉ điều tra"
          required
          value={form.investigationRestore.suspendedDecisionInfoLine}
          onChange={(value) =>
            patch("investigationRestore", "suspendedDecisionInfoLine", value)
          }
        />
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Field
            label="Tên vụ án"
            required
            value={form.investigationRestore.caseTitle}
            onChange={(value) =>
              patch("investigationRestore", "caseTitle", value)
            }
          />
          ...
          <Field
            label="Tội danh"
            required
            value={form.investigationRestore.offenseName}
            onChange={(value) =>
              patch("investigationRestore", "offenseName", value)
            }
          />
        </div>
        ...
        <Field
          ...
          required
          multiline
          value={form.investigationRestore.reasonLine}
          onChange={(value) =>
            patch("investigationRestore", "reasonLine", value)
          }
        />
      </SectionCard>
