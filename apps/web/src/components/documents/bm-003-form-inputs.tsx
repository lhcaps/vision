  ...
  return (
    <div className="space-y-5">
      ...
      >
        <div className="grid gap-4 md:grid-cols-2">
          <Field
            label="Kiểm sát viên được phân công"
            value={form.sourceAssignment.prosecutorName}
            onChange={(value) => updateSource("prosecutorName", value, true)}
          />
          <Field
            label="Chức danh Kiểm sát viên"
            value={form.sourceAssignment.prosecutorTitle}
            onChange={(value) => updateSource("prosecutorTitle", value, true)}
          />
          <Field
            label="Cơ quan cung cấp / giải quyết nguồn tin"
            value={form.sourceAssignment.sourceProvider}
            onChange={(value) => updateSource("sourceProvider", value, true)}
          />
          <Field
            label="Tóm tắt vụ việc / nguồn tin"
            value={form.sourceAssignment.caseSummary}
            onChange={(value) => updateSource("caseSummary", value, true)}
          />
        </div>
        ...
        {form.sourceAssignment.hasInspectorAssistant ? (
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <Field
              label="Kiểm tra viên giúp việc"
              value={form.sourceAssignment.inspectorName}
              onChange={(value) => updateSource("inspectorName", value, true)}
            />
            <Field
              label="Chức danh Kiểm tra viên"
              value={form.sourceAssignment.inspectorTitle}
              onChange={(value) => updateSource("inspectorTitle", value, true)}
            />
          </div>
        ) : null}
      </SectionCard>
