...
type Bm161Form = {
  ...
  content: { extractedPersonName: string; extractedPersonDob: string; extractedPersonAddress: string; extractionReason: string; extractionContent: string; article1Line: string };
  recipients: { line1: string; line2: string; archiveLine: string };
  signature: { signMode: string; positionTitle: string; signerName: string };
};

type Bm161Props = { documentId: string | number; onSaved?: () => void };
...
const EMPTY_FORM: Bm161Form = {
  ...
  content: { extractedPersonName: "", extractedPersonDob: "", extractedPersonAddress: "", extractionReason: "", extractionContent: "", article1Line: "" },
...
function normalizeForm(payload: Record<string, unknown> | null): Bm161Form {
  ...
  return {
    ...
    content: { extractedPersonName: nested(payload, "content.extractedPersonName") || "", extractedPersonDob: nested(payload, "content.extractedPersonDob") || "", extractedPersonAddress: nested(payload, "content.extractedPersonAddress") || "", extractionReason: nested(payload, "content.extractionReason") || "", extractionContent: nested(payload, "content.extractionContent") || "", article1Line: nested(payload, "content.article1Line") || "" },
...
export function Bm161FormInputsPanel({ documentId, onSaved }: Bm161Props) {
  const [form, setForm] = useState<Bm161Form>(EMPTY_FORM);
  const [saving, setSaving] = useState(false);
  const [msg, setMsg] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const patch = (section: keyof Bm161Form, key: string, value: string) => setForm(f => ({ ...f, [section]: { ...f[section], [key]: value } }));
  ...
  return (<div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm"><div className="flex items-center justify-between border-b border-slate-200 pb-4"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-161</p><h2 className="mt-1 text-xl font-bold text-slate-950">Phiếu yêu cầu trích xuất</h2></div><div className="flex gap-2"><button className="rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50" onClick={() => void reload()}>Tải lại</button><button className="rounded-xl bg-blue-600 px-3.5 py-2.5 text-sm font-bold text-white hover:bg-blue-700" onClick={() => void handleSave()} disabled={saving}>{saving ? "Đang lưu..." : "Lưu dữ liệu"}</button></div></div>{msg ? <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-700">{msg}</div> : null}{error ? <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-medium text-red-700">{error}</div> : null}<SectionCard title="1. Header biểu mẫu"><Field label="Viện kiểm sát cấp trên" value={form.agency.parentName} onChange={v => patch("agency", "parentName", v)} /><Field label="Viện kiểm sát ban hành" value={form.agency.name} onChange={v => patch("agency", "name", v)} /><Field label="Số phiếu yêu cầu" value={form.document.documentCode} onChange={v => patch("document", "documentCode", v)} /><Field label="Địa danh" value={form.agency.issuePlace} onChange={v => patch("agency", "issuePlace", v)} /><Field label="Ngày ban hành" type="date" value={form.document.issueDate} onChange={v => patch("document", "issueDate", v)} /><Field label="Chủ thể ban hành" value={form.official.issuerTitle} onChange={v => patch("official", "issuerTitle", v)} className="md:col-span-2" /></SectionCard><SectionCard title="2. Thông tin người được yêu cầu trích xuất"><Field label="Họ tên người được trích xuất" value={form.content.extractedPersonName} onChange={v => patch("content", "extractedPersonName", v)} /><Field label="Ngày sinh" value={form.content.extractedPersonDob} onChange={v => patch("content", "extractedPersonDob", v)} /><Field label="Địa chỉ" value={form.content.extractedPersonAddress} onChange={v => patch("content", "extractedPersonAddress", v)} className="md:col-span-2" /></SectionCard><SectionCard title="3. Nội dung yêu cầu trích xuất"><Field label="Lý do trích xuất" value={form.content.extractionReason} onChange={v => patch("content", "extractionReason", v)} className="md:col-span-2" /><Field label="Nội dung yêu cầu" multiline value={form.content.extractionContent} onChange={v => patch("content", "extractionContent", v)} className="md:col-span-2" /><Field label="Điều khoản" multiline value={form.content.article1Line} onChange={v => patch("content", "article1Line", v)} className="md:col-span-2" /></SectionCard><SectionCard title="4. Nơi nhận"><Field label="Nơi nhận 1" value={form.recipients.line1} onChange={v => patch("recipients", "line1", v)} /><Field label="Nơi nhận 2" value={form.recipients.line2} onChange={v => patch("recipients", "line2", v)} /><Field label="Lưu hồ sơ" value={form.recipients.archiveLine} onChange={v => patch("recipients", "archiveLine", v)} className="md:col-span-2" /></SectionCard><SectionCard title="5. Chữ ký"><Field label="Chế độ ký" value={form.signature.signMode} onChange={v => patch("signature", "signMode", v)} /><Field label="Chức vụ ký" value={form.signature.positionTitle} onChange={v => patch("signature", "positionTitle", v)} /><Field label="Người ký" value={form.signature.signerName} onChange={v => patch("signature", "signerName", v)} /></SectionCard></div>);
}
