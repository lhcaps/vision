...
const EMPTY_FORM: Bm118Form = {
  ...
  decision: { procedureArticlesLine: "Căn cứ các điều 36, 37 và 172 của Bộ luật Tố tụng hình sự;", caseTitle: "", offenseName: "", accusedFullName: "", suspendedDecisionCode: "", suspendedDecisionDate: "", suspendedDecisionAgency: "", reasonLine: "" },
...
function buildReasonLine(form: Bm118Form): string {
  const d = form.decision;
  if (!d.accusedFullName || !d.caseTitle) return "";
  return `Xét thấy QĐ số ${d.suspendedDecisionCode.trim()}${d.suspendedDecisionDate ? ` ngày ${cleanText(d.suspendedDecisionDate)}` : ""} của ${d.suspendedDecisionAgency.trim()} về việc tạm đình chỉ điều tra VAHS đối với bị can ${d.accusedFullName.trim()} trong vụ án ${d.caseTitle.trim()} về tội "${d.offenseName.trim()}" không có căn cứ, cần phục hồi điều tra VA đối với bị can.`;
}
...
function normalizeFormInputs(payload: RenderPayload | null): Bm118Form {
  ...
  return {
    ...
    decision: {
      procedureArticlesLine: nested(payload, "decision.procedureArticlesLine") || f.decision.procedureArticlesLine,
      ...
      accusedFullName: nested(payload, "decision.accusedFullName") || nested(payload, "accused.fullName") || "",
      suspendedDecisionCode: nested(payload, "decision.suspendedDecisionCode") || "",
      suspendedDecisionDate: nested(payload, "decision.suspendedDecisionDate") || "",
      suspendedDecisionAgency: nested(payload, "decision.suspendedDecisionAgency") || "",
...
function validateForm(form: Bm118Form): string[] {
  const reqs: [string, string][] = [
    ...
    ["Căn cứ pháp luật", form.decision.procedureArticlesLine], ["Họ tên bị can", form.decision.accusedFullName],
    ["Tên vụ án", form.decision.caseTitle], ["Số QĐ tạm đình chỉ", form.decision.suspendedDecisionCode],
...
function buildSaveBody(form: Bm118Form) {
  ...
  return {
    ...
    decision: { procedureArticlesLine: form.decision.procedureArticlesLine, caseTitle: form.decision.caseTitle, offenseName: form.decision.offenseName, accusedFullName: form.decision.accusedFullName, suspendedDecisionCode: form.decision.suspendedDecisionCode, suspendedDecisionDate: form.decision.suspendedDecisionDate, suspendedDecisionAgency: form.decision.suspendedDecisionAgency, reasonLine: buildReasonLine(form) },
...
export function Bm118FormInputsPanel({ documentId, onSaved }: { documentId: string | number; onSaved?: () => void | Promise<void> }) {
  ...
  const patch = <T extends keyof Bm118Form>(section: T, key: keyof Bm118Form[T], value: string) => {
  ...
  return (
    <div className="space-y-5 rounded-3xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
      <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-600">BM-118</p>
          <h2 className="mt-1 text-xl font-bold text-slate-950">QĐ phục hồi điều tra VA đối với bị can</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">Form nhập liệu placeholder cho BM-118.</p>
        </div>
      ...
      <SectionCard title="2. Thông tin phục hồi điều tra" description="Nhập thông tin QĐ tạm đình chỉ VAHS cần phục hồi.">
        ...
        <Field label="Họ tên bị can" required value={form.decision.accusedFullName} onChange={v => patch("decision", "accusedFullName", v)} />
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Field label="Số QĐ tạm đình chỉ" required value={form.decision.suspendedDecisionCode} onChange={v => patch("decision", "suspendedDecisionCode", v)} />
          <Field label="Ngày QĐ tạm đình chỉ" value={form.decision.suspendedDecisionDate} onChange={v => patch("decision", "suspendedDecisionDate", v)} />
        </div>
        <Field label="Cơ quan ban hành QĐ" value={form.decision.suspendedDecisionAgency} onChange={v => patch("decision", "suspendedDecisionAgency", v)} />
